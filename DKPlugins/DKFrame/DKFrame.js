...
function DKFrame_Iframe(title, url, width, height)
{
	...
	DKCef_SetUrl(url, currentBrowser);
	DKFrame_CreateResize(frame);
	DKCef_SetFocus();
	return iframe;
}
...
function DKFrame_CreateFrame(title, width, height)
{
	...
	var newheight = parseInt(height)+21;
	var newtop = parseInt((window_height / 2) - (newheight / 2) - 21);
	var newleft = parseInt((window_width / 2) - (width / 2));
