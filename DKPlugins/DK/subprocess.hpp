...
namespace subprocess
{
	...
	public:
		...
		popen(const std::string& cmd, std::vector<std::string> argv) : in_filebuf(nullptr), out_filebuf(nullptr), err_filebuf(nullptr), in_stream(nullptr), out_stream(nullptr), err_stream(nullptr) {
			if (pipe(in_pipe)  == -1 || pipe(out_pipe) == -1 || pipe(err_pipe) == -1 )
				throw std::system_error(errno, std::system_category());
			run(cmd, argv);
		}

		popen(const std::string& cmd, std::vector<std::string> argv, std::ostream& pipe_stdout) : in_filebuf(nullptr), out_filebuf(nullptr), err_filebuf(nullptr), in_stream(nullptr), out_stream(nullptr), err_stream(nullptr) {
	...
	private:
		...
		void child(const std::vector<std::string>& argv) {  
			if(dup2(in_pipe[READ], STDIN_FILENO)    == -1 || dup2(out_pipe[WRITE], STDOUT_FILENO) == -1 || dup2(err_pipe[WRITE], STDERR_FILENO) == -1 ) {
