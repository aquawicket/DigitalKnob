...
function DKBuild_DoResults(){
	...
	*/
	...
	if(OS === "iossim32"){
		...
		CPP_DKFile_MkDir(app_path+OS)
		CPP_DKFile_ChDir(app_path+OS)
		let rtvalue = CPP_DK_Execute(CMAKE+" -G \"Xcode\" "+cmake_string+"-DCMAKE_TOOLCHAIN_FILE="+DIGITALKNOB+"DKCMake/ios.toolchain.cmake -DIOS_PLATFORM=SIMULATOR "+DIGITALKNOB+"DK")
		if(rtvalue.indexOf("errors occurred!") > -1) 
			return
		if(TYPE === "Debug" || TYPE === "ALL")
			//CPP_DKFile_Rename(app_path+OS+"/Debug/"+APP+".app", app_path+OS+"/Debug/"+APP+"_OLD.app", true)
	...
	if(OS === "iossim64"){
		...
		CPP_DKFile_MkDir(app_path+OS)
		CPP_DKFile_ChDir(app_path+OS)
		let rtvalue = CPP_DK_Execute(CMAKE+" -G \"Xcode\" "+cmake_string+"-DCMAKE_TOOLCHAIN_FILE="+DIGITALKNOB+"DK/DKCMake/ios.toolchain.cmake -DPLATFORM=SIMULATOR64 "+DIGITALKNOB+"DK")
		if(rtvalue.indexOf("errors occurred!") > -1) 
			return
		if(TYPE === "Debug" || TYPE === "ALL")
			//CPP_DKFile_Rename(app_path+OS+"/Debug/"+APP+".app", app_path+OS+"/Debug/"+APP+"_OLD.app", true)
	...
	if(OS === "android64"){
		...
		if(CPP_DK_GetOS() === "Windows")
			let rtvalue = CPP_DK_Execute(CMAKE+" -G \"Visual Studio 16 2019\" -A ARM64 -DCMAKE_TOOLCHAIN_FILE="+DIGITALKNOB+"/DKCmake/"+android.toolchain.cmake -DANDROIDNDK="+ANDROIDNDK+" -DANDROID_ABI=arm64-v8a -DANDROID_NATIVE_API_LEVEL=24 –DCMAKE_SYSTEM_NAME=ANDROID "+cmake_string+DIGITALKNOB+"DK")
		if(CPP_DK_GetOS() === "Linux" || CPP_DK_GetOS() === "Mac")
			rtvalue = CPP_DK_Execute(CMAKE+" -G \"Unix Makefiles\" "+cmake_string+DIGITALKNOB+"DK")
		if(rtvalue.indexOf("errors occurred!") > -1) 
			return 
