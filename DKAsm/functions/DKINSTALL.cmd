...
%setlocal%
	...
	set "source=dk_mainArgs"
	set "Target_App=%source%"
	...
	if defined Windows_X86_Gcc (
		...
		%nasm_exe% -g -f elf32 %source%.asm -o "%Target_Tuple%/%Target_App%.o"
	) else if defined Windows_X86_64_Gcc (
		set "PATH=%msys2%/mingw64/bin;%PATH%"
		%nasm_exe% -g -f elf64 %source%.asm -o "%Target_Tuple%/%Target_App%.o"		
