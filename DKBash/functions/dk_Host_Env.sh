...
# dk_Host_Env()
#
#	  Host_Env = Cmd, Powershell, Pwsh, Sh, Bash, Zsh 
#
dk_Host_Env() {
	dk_debugFunc 0 1
	...
	###### GET ######	
		...
		if [ -z "${Host_Env-}" ]; then
			[ -n "${ComSpec-}" ] && Host_Env="${ComSpec}"
			Host_Env=$(basename ${Host_Env})
		fi
		
		if [ -z "${Host_Env-}" ]; then
			[ -n "${BASH-}" ] && Host_Env="${BASH}"
			Host_Env=$(basename ${Host_Env})
		fi
