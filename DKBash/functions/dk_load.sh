...
dk_load (){
	...
	if [ ! -e ${funcPath} ]; then
		if [[ "${funcName}" =~ ^dk_[a-zA-Z0-9]+ ]]; then
			[ -e "${DKBASH_FUNCTIONS_DIR}/dk_download.sh" ] || dk_call ${dksudo} wget -P ${DKBASH_FUNCTIONS_DIR} ${DKHTTP_DKBASH_FUNCTIONS_DIR}/dk_download.sh
			[ -e "${DKBASH_FUNCTIONS_DIR}/dk_download.sh" ] || dk_call ${dksudo} curl -Lo ${DKBASH_FUNCTIONS_DIR}/dk_download.sh ${DKHTTP_DKBASH_FUNCTIONS_DIR}/dk_download.sh
