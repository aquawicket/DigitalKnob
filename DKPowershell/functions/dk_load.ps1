...
if(!$dk_load_ps1){ $dk_load_ps1 = 1; } else{ return; } #include guard
...
function Global:dk_load ($var) {
	...
	if(!(Test-Path ${funcPath})){
		if(!(Test-Path "${DKPOWERSHELL_FUNCTIONS_DIR}/dk_download.ps1")){ 
			Invoke-WebRequest -URI "$DKHTTP_DKPOWERSHELL_FUNCTIONS_DIR/dk_download.ps1" -OutFile "${DKPOWERSHELL_FUNCTIONS_DIR}/dk_download.ps1";
		}
		if(!(Test-Path dk_download)){ 
