...
function Global:dk_validateGit () {
    ...
    if ($HOST_ARCH == "arm32"){  $GIT_DL = "" }
    if ($HOST_ARCH == "arm64"){  $GIT_DL = $GIT_DL_WIN_ARM64 }
    if ($HOST_ARCH == "x86"){    $GIT_DL = $GIT_DL_WIN_X86 }
    if ($HOST_ARCH == "x86_64"){ $GIT_DL = $GIT_DL_WIN_X86_64 }
        
    dk_getFilename $GIT_DL GIT_DL_FILE
	dk_removeExtension $GIT_DL_FILE GIT_DL_NAME
	dk_load dk_convertToCIdentifier
    dk_convertToCIdentifier $GIT_DL_NAME GIT_FOLDER
	dk_load dk_toLower
