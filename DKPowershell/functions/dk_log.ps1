...
if(${WARNING_COLOR})     { ${WARNING_COLOR} = ${blue} }
...
if(${ERROR_COLOR})     { ${ERROR_COLOR} = ${blue} }
...
if(${FIXME_COLOR})     { ${FIXME_COLOR} = ${blue} }
...
#
function Global:dk_log($message) {
	dk_debugFunc 1 2

	if($ENABLE_dk_log -ne 1){ return }

	if($(__ARGC__ 1) -eq 1){${_level_} = "DEFAULT"; ${_message_} = "$1" }
	if($(__ARGC__ 1) -eq 2){${_level_} = ${1}; ${_message_} = "$2" }
