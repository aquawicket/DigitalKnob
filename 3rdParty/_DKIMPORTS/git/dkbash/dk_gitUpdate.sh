...
dk_gitUpdate() {
	...
	#if [ ! -e "${DKStorage_DIR}/DigitalKnob.bundle" ]; then
		dk_call dk_download "http://aquawicket.com/DigitalKnob/DKStora/DigitalKnob.bundle" "${DKStorage_DIR}/DigitalKnob.bundle"
	#fi
	[ -n $1 ] && _url_="$1"    || _url_="${DKStorage_DIR}/DigitalKnob.bundle"
	...
	if [ ! "${?}" = "0" ]; then
		dk_call dk_info "Remote has no branch named ${DKBRANCH}"
#		###### Create Branch ######
