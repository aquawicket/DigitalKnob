...
## RML minimal
WIN_dk_queueCommand			(${DKCMAKE_BUILD} "-DCMAKE_CXX_FLAGS=/DRMLUI_STATIC_LIB /DGLEW_STATIC /I${RMLUI}/Include /I${SDL}/include /I${GLEW}/include" -DBUILD_SHARED_LIBS=OFF -DBUILD_SAMPLES=OFF -DBUILD_TESTING=OFF -DENABLE_SVG_PLUGIN=OFF -DENABLE_LOTTIE_PLUGIN=OFF -DBUILD_LUA_BINDINGS=ON ${FREETYPE_CMAKE} ${LUA_CMAKE} ${RMLUI})
...
MAC_dk_queueCommand			(${DKCMAKE_BUILD} -DBUILD_SHARED_LIBS=OFF -DENABLE_PRECOMPILED_HEADERS=OFF -DBUILD_LUA_BINDINGS=ON ${FREETYPE_CMAKE} ${LUA_CMAKE} ${RMLUI})
IOS_dk_queueCommand			(${DKCMAKE_BUILD} -DBUILD_SHARED_LIBS=OFF -DENABLE_PRECOMPILED_HEADERS=OFF -DBUILD_LUA_BINDINGS=OFF ${FREETYPE_CMAKE} ${LUA_CMAKE} ${RMLUI})
IOSSIM_dk_queueCommand		(${DKCMAKE_BUILD} -DBUILD_SHARED_LIBS=OFF -DENABLE_PRECOMPILED_HEADERS=OFF -DBUILD_LUA_BINDINGS=OFF ${FREETYPE_CMAKE} ${LUA_CMAKE} ${RMLUI})
LINUX_dk_queueCommand		(${DKCMAKE_BUILD} -DBUILD_LUA_BINDINGS=ON ${FREETYPE_CMAKE} ${LUA_CMAKE} ${RMLUI})
RASPBERRY_dk_queueCommand	(${DKCMAKE_BUILD} -DBUILD_LUA_BINDINGS=ON ${FREETYPE_CMAKE} ${LUA_CMAKE} ${RMLUI})
ANDROID_dk_queueCommand		(${DKCMAKE_BUILD} "-DCMAKE_CXX_FLAGS=-DRMLUI_STATIC_LIB -DCHOBO_FLAT_MAP_NO_THROW -DRMLUI_USE_CUSTOM_RTTI -std=c++1z" -DENABLE_PRECOMPILED_HEADERS=OFF -DDISABLE_RTTI_AND_EXCEPTIONS=ON -DBUILD_LUA_BINDINGS=ON ${FREETYPE_CMAKE} ${LUA_CMAKE} ${RMLUI})
