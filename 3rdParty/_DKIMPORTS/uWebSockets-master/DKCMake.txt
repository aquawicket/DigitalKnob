...
### INSTALL ###
## DKINSTALL(https://github.com/uNetworking/uWebSockets/archive/master.zip uWebSockets-master)
DKSET(UWEBSOCKETS ${3RDPARTY}/uWebSockets-master)
...
WIN32_VS_DEBUG(uWebSockets-master µWebSockets.sln uWS)
WIN32_VS_RELEASE(uWebSockets-master µWebSockets.sln uWS)
...
WIN64_VS_DEBUG(uWebSockets-master uWebSockets.sln uWS)
WIN64_VS_RELEASE(uWebSockets-master uWebSockets.sln uWS)

DKSETPATH(${UWEBSOCKETS}/${OS})
MAC64_COMMAND(${CMAKE_COMMAND} -G "Xcode" -DCMAKE_OSX_ARCHITECTURES=x86_64 ${UWEBSOCKETS})
MAC64_XCODE_DEBUG(uWebSockets-master libuWS)
MAC64_XCODE_RELEASE(uWebSockets-master libuWS)

DKSETPATH(${UWEBSOCKETS}/${OS})
IOS_COMMAND(${CMAKE_COMMAND} -G "Xcode" -DCMAKE_TOOLCHAIN_FILE=${DKCMAKE}/iOS.cmake -DIOS_PLATFORM=OS ${UWEBSOCKETS})
IOS_XCODE_DEBUG(uWebSockets-master libuWS)
IOS_XCODE_RELEASE(uWebSockets-master libuWS)

DKSETPATH(${UWEBSOCKETS}/${OS})
IOSSIM_COMMAND(${CMAKE_COMMAND} -G "Xcode" -DCMAKE_TOOLCHAIN_FILE=${DKCMAKE}/iOS.cmake -DIOS_PLATFORM=SIMULATOR ${UWEBSOCKETS})
IOSSIM_XCODE_DEBUG(uWebSockets-master libuWS)
IOSSIM_XCODE_RELEASE(uWebSockets-master libuWS)
...
ANDROID_NDK_DEBUG(uWebSockets-master)
ANDROID_NDK_RELEASE(uWebSockets-master)