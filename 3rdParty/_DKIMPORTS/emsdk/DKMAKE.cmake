...
if(WIN_HOST)
	dk_depend(cmake)
	execute_process(COMMAND ${CMD_EXE} /c ${CMAKE_EXE} -E env PATH=${PYTHON3_DIR_WIN}	"${EMSDK_DIR}/emsdk.bat"  install latest 			COMMAND_ECHO STDOUT)
else()
...
if(WIN_HOST)
	execute_process(COMMAND ${CMD_EXE} /c ${CMAKE_EXE} -E env PATH=${PYTHON3_DIR_WIN}	"${EMSDK_DIR}/emsdk.bat" activate latest 			COMMAND_ECHO STDOUT)  # --permanent
else()
...
if(WIN_HOST)
	execute_process(COMMAND ${CMD_EXE} /c call											"${EMSDK_DIR}/emsdk_env.bat" 						COMMAND_ECHO STDOUT)
else()
...
#if(WIN_HOST)
#	execute_process(COMMAND ${CMD_EXE} /c call 									"${EMSDK_DIR}/emsdk.bat" install mingw_4.6.2_32bit		COMMAND_ECHO STDOUT)
#	execute_process(COMMAND ${CMD_EXE} /c call 									"${EMSDK_DIR}/emsdk.bat" activate mingw_4.6.2_32bit		COMMAND_ECHO STDOUT)
#endif()
...
dk_set				(EMSCRIPTEN_TOOLCHAIN_FILE 		${EMSDK_DIR}/upstream/emscripten/cmake/Modules/Platform/Emscripten.cmake)
#if(WIN_HOST)
#	dk_set			(EMSCRIPTEN_GENERATOR 			"MinGW Makefiles")
#else()
#	dk_set			(EMSCRIPTEN_GENERATOR 			"Unix Makefiles")
#endif()
#dk_set				(EMSCRIPTEN_AR 					"${EMSDK_DIR}/upstream/emscripten/emar${bat}")				# Name of archiving tool for static libraries.
#dk_set				(EMSCRIPTEN_MAKE_PROGRAM		"${EMSDK_DIR}/upstream/emscripten/emmake${bat}")
#dk_set				(EMSCRIPTEN_C_COMPILER		    "${EMSDK_DIR}/upstream/emscripten/emcc${bat}")
#dk_set				(EMSCRIPTEN_C_COMPILER		    "${EMSDK_DIR}/upstream/bin/clang.exe")
#dk_set				(EMSCRIPTEN_CXX_COMPILER	    "${EMSDK_DIR}/upstream/emscripten/em++${bat}")
#dk_set				(EMSCRIPTEN_CXX_COMPILER	    "${EMSDK_DIR}/upstream/bin/clang++.exe")
#dk_set				(EMSCRIPTEN_NM 					"${EMSDK_DIR}/upstream/emscripten/emnm${bat}")	
#dk_set				(EMSCRIPTEN_RANLIB 				"${EMSDK_DIR}/upstream/emscripten/emranlib${bat}")
#dk_set				(EMSCRIPTEN_C_COMPILER_AR 		"${EMSCRIPTEN_AR}")
#dk_set				(EMSCRIPTEN_CXX_COMPILER_AR 	"${EMSCRIPTEN_AR}")
#dk_set				(EMSCRIPTEN_C_COMPILER_RANLIB 	"${EMSCRIPTEN_RANLIB}")
#dk_set				(EMSCRIPTEN_CXX_COMPILER_RANLIB	"${EMSCRIPTEN_RANLIB}")
