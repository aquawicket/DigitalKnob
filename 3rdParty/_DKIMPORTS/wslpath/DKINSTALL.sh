...
DKINSTALL() {
	...
	(command -v "${wslpath_exe-}" /c ver 1>/dev/null) && return $?;
	
	[ ! -e "${wslpath_exe-}" ] && wslpath_exe=$(command -v cmd)
	[ ! -e "${wslpath_exe-}" ] && wslpath_exe="C:/Windows/System32/cmd.exe"
	...
	[ ! -e "${wslpath_exe-}" ] && dk_call dk_installPackage cmd
	(command -v ${wslpath_exe} /c ver 1>/dev/null) || { dk_call dk_error "wslpath_exe:${wslpath_exe} failed to run"; return $?; }
