...
function(DKINSTALL)
	...
	#execute_process(COMMAND "${cmake.exe}" --version RESULT_VARIABLE exit_code OUTPUT_QUIET)
	execute_process(COMMAND cmd /c "${cmake.exe}" --version RESULT_VARIABLE exit_code OUTPUT_QUIET)
	if(NOT "${exit_code}" STREQUAL "0")
		dk_pathToCmake("$ENV{cmake.exe}" cmake.exe)
	endif()
	...
	if(NOT "${exit_code}" STREQUAL "0")
		...
		if(${Host_Os} STREQUAL "Windows")
			dk_pathToCmake("${cmake.exe}/bin/cmake.exe" cmake.exe)
		else() 
			dk_pathToCmake("${cmake.exe}/bin/cmake" cmake.exe)
		endif()
	endif()
	
	execute_process(COMMAND cmd /c "${cmake.exe}" --version RESULT_VARIABLE exit_code)
	if(NOT "${exit_code}" STREQUAL "0")
		if(EXISTS "${cmake.exe}")
			dk_error("cmake.exe:'${cmake.exe}' FAILED")
		else()
			dk_error("cmake.exe:'${cmake.exe}' NOT FOUND")
		endif()
		return()
	endif()
	...
	#return
	dk_firewallAllow("${cmake.exe}")
	dk_set(cmake.exe "${cmake.exe}")
	#dk_debug("cmake.exe = ${cmake.exe}")
