# http://giflib.sourceforge.net/
#
# https://sourceforge.net/projects/giflib/files/giflib-5.1.1.tar.gz

if(WIN_32)
	DKDEPEND(mingw32)
	DKDEPEND(msys)
endif()
if(WIN_64)
	DKDEPEND(mingw64)
	DKDEPEND(msys)
endif()


### VERSION ###
DKSET(GIF_VERSION 5.1.1)
DKSET(GIF_DL https://sourceforge.net/projects/giflib/files/giflib-5.1.1.tar.gz)




### INSTALL ###
DKSET(GIF_NAME giflib-${GIF_VERSION})
DKSET(GIF ${3RDPARTY}/${GIF_NAME})
DKINSTALL(${GIF_DL} giflib ${GIF})



### DKPLUGINS LINK ###
DKINCLUDE(${GIF}/lib)
WIN_DEBUG_DKLIB(${GIF}/${OS}/${DEBUG_DIR}/lib/.libs/libgif.a)
WIN_RELEASE_DKLIB(${GIF}/${OS}/${RELEASE_DIR}/lib/.libs/libgif.a)
APPLE_DEBUG_DKLIB(${GIF}/${OS}/${DEBUG_DIR}/lib/.libs/libgif.a)
APPLE_RELEASE_DKLIB(${GIF}/${OS}/${RELEASE_DIR}/lib/.libs/libgif.a)
LINUX_DEBUG_DKLIB(${GIF}/${OS}/${DEBUG_DIR}/lib/.libs/libgif.a)
LINUX_RELEASE_DKLIB(${GIF}/${OS}/${RELEASE_DIR}/lib/.libs/libgif.a)
RASPBERRY_DEBUG_DKLIB(${GIF}/${OS}/${DEBUG_DIR}/lib/.libs/libgif.a)
RASPBERRY_RELEASE_DKLIB(${GIF}/${OS}/${RELEASE_DIR}/lib/.libs/libgif.a)
ANDROID_DEBUG_DKLIB(${GIF}/${OS}/${DEBUG_DIR}/libgiflib.a)
ANDROID_RELEASE_DKLIB(${GIF}/${OS}/${RELEASE_DIR}/libgiflib.a)



### 3RDPARTY LINK ###
WIN_DKSET(GIF_CMAKE -DGIF_INCLUDE_DIR=${GIF}/lib -DGIF_INCLUDE_DIR2=${GIF}/${OS} -DGIF_LIBRARY=${GIF}/${OS}/${RELEASE_DIR}/lib/.libs/libgif.a)
APPLE_DKSET(GIF_CMAKE -DGIF_INCLUDE_DIR=${GIF}/lib -DGIF_INCLUDE_DIR2=${GIF}/${OS} -DGIF_LIBRARY=${GIF}/${OS}/${RELEASE_DIR}/lib/.libs/libgif.a)
LINUX_DKSET(GIF_CMAKE -DGIF_INCLUDE_DIR=${GIF}/lib -DGIF_INCLUDE_DIR2=${GIF}/${OS} -DGIF_LIBRARY=${GIF}/${OS}/${RELEASE_DIR}/lib/.libs/libgif.a)
RASPBERRY_DKSET(GIF_CMAKE -DGIF_INCLUDE_DIR=${GIF}/lib -DGIF_INCLUDE_DIR2=${GIF}/${OS} -DGIF_LIBRARY=${GIF}/${OS}/${RELEASE_DIR}/lib/.libs/libgif.a)
ANDROID_DKSET(GIF_CMAKE -DGIF_INCLUDE_DIR=${GIF}/lib -DGIF_INCLUDE_DIR2=${GIF}/${OS} -DGIF_LIBRARY=${GIF}/${OS}/${RELEASE_DIR}/libgiflib.a)	


### COMPILE ###
WIN32_DEBUG_DKSETPATH(${GIF}/${OS}/${DEBUG_DIR})
WIN32_DEBUG_MSYS(${DKCONFIGURE_BUILD})
WIN32_DEBUG_MSYS(make)
WIN32_RELEASE_DKSETPATH(${GIF}/${OS}/${RELEASE_DIR})
WIN32_RELEASE_MSYS(${DKCONFIGURE_BUILD})
WIN32_RELEASE_MSYS(make)


WIN64_DEBUG_DKSETPATH(${GIF}/${OS}/${DEBUG_DIR})
WIN64_DEBUG_MSYS(${DKCONFIGURE_BUILD})
WIN64_DEBUG_MSYS(make)
WIN64_RELEASE_DKSETPATH(${GIF}/${OS}/${RELEASE_DIR})
WIN64_RELEASE_MSYS(${DKCONFIGURE_BUILD})
WIN64_RELEASE_MSYS(make)


MAC_DEBUG_DKSETPATH(${GIF}/${OS}/${DEBUG_DIR})
MAC_DEBUG_DKQCOMMAND(${DKCONFIGURE_BUILD})
MAC_DEBUG_DKQCOMMAND(make)
MAC_RELEASE_DKSETPATH(${GIF}/${OS}/${RELEASE_DIR})
MAC_RELEASE_DKQCOMMAND(${DKCONFIGURE_BUILD})
MAC_RELEASE_DKQCOMMAND(make)


IOS_DEBUG_DKSETPATH(${GIF}/${OS}/${DEBUG_DIR})
IOS_DEBUG_DKQCOMMAND(${DKCONFIGURE_BUILD})
IOS_DEBUG_DKQCOMMAND(make)
IOS_RELEASE_DKSETPATH(${GIF}/${OS}/${RELEASE_DIR})
IOS_RELEASE_DKQCOMMAND(${DKCONFIGURE_BUILD})
IOS_RELEASE_DKQCOMMAND(make)


IOSSIM_DEBUG_DKSETPATH(${GIF}/${OS}/${DEBUG_DIR})
IOSSIM_DEBUG_DKQCOMMAND(${DKCONFIGURE_BUILD})
IOSSIM_DEBUG_DKQCOMMAND(make)
IOSSIM_RELEASE_DKSETPATH(${GIF}/${OS}/${RELEASE_DIR})
IOSSIM_RELEASE_DKQCOMMAND(${DKCONFIGURE_BUILD})
IOSSIM_RELEASE_DKQCOMMAND(make)


LINUX_DEBUG_DKSETPATH(${GIF}/${OS}/${DEBUG_DIR})
LINUX_DEBUG_DKQCOMMAND(${DKCONFIGURE_BUILD})
LINUX_DEBUG_DKQCOMMAND(make)
LINUX_RELEASE_DKSETPATH(${GIF}/${OS}/${RELEASE_DIR})
LINUX_RELEASE_DKQCOMMAND(${DKCONFIGURE_BUILD})
LINUX_RELEASE_DKQCOMMAND(make)


RASPBERRY_DEBUG_DKSETPATH(${GIF}/${OS}/${DEBUG_DIR})
RASPBERRY_DEBUG_DKQCOMMAND(${DKCONFIGURE_BUILD})
RASPBERRY_DEBUG_DKQCOMMAND(make)
RASPBERRY_RELEASE_DKSETPATH(${GIF}/${OS}/${RELEASE_DIR})
RASPBERRY_RELEASE_DKQCOMMAND(${DKCONFIGURE_BUILD})
RASPBERRY_RELEASE_DKQCOMMAND(make)



DKSETPATH(${GIF}/${BUILD_DIR})
ANDROID_DKQCOMMAND(${DKCMAKE_BUILD} ${GIF})
ANDROID_VS(${GIF_NAME} giflib.sln giflib)