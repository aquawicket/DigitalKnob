### VERSION ###
DKSET(JPEG_VERSION jpeg-6b)

### INSTALL ###
DKINSTALL(https://www.sac.sk/download/graph/jpegsr6b.zip jpeg ${JPEG_VERSION})
...
WIN32_VS_DEBUG(${JPEG_VERSION} jpeg.sln jpeg-static)
WIN32_VS_RELEASE(${JPEG_VERSION} jpeg.sln jpeg-static)
...
WIN64_VS_DEBUG(${JPEG_VERSION} jpeg.sln jpeg-static)
WIN64_VS_RELEASE(${JPEG_VERSION} jpeg.sln jpeg-static)

DKSETPATH(${JPEG}/${OS})
MAC_COMMAND(${CMAKE_COMMAND} -G "Xcode" -DCMAKE_OSX_ARCHITECTURES=x86_64 ${JPEG})
MAC_XCODE_DEBUG(${JPEG_VERSION} jpeg-static)
MAC_XCODE_RELEASE(${JPEG_VERSION} jpeg-static)

DKSETPATH(${JPEG}/${OS})
IOS_COMMAND(${CMAKE_COMMAND} -G "Xcode" -DCMAKE_TOOLCHAIN_FILE=${DKCMAKE}/iOS.cmake -DIOS_PLATFORM=OS ${JPEG})
IOS_XCODE_DEBUG(${JPEG_VERSION} jpeg-static)
IOS_XCODE_RELEASE(${JPEG_VERSION} jpeg-static)

DKSETPATH(${JPEG}/${OS})
IOSSIM_COMMAND(${CMAKE_COMMAND} -G "Xcode" -DCMAKE_TOOLCHAIN_FILE=${DKCMAKE}/iOS.cmake -DIOS_PLATFORM=SIMULATOR64 ${JPEG})
IOSSIM_XCODE_DEBUG(${JPEG_VERSION} jpeg-static)
IOSSIM_XCODE_RELEASE(${JPEG_VERSION} jpeg-static)
