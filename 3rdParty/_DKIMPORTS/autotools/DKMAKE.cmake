# http://autogen.sourceforge.net
# https://macappstore.org/autogen/
# https://www.xmodulo.com/fix-failed-to-run-aclocal.html
# https://thoughtbot.com/blog/the-magic-behind-configure-make-make-install

MAC_HOST_dk_queueCommand(brew install autogen)
MAC_HOST_dk_queueCommand(brew install autoconf)
MAC_HOST_dk_queueCommand(brew install automake)

if(WIN_HOST)
	dk_depend(msys2)
	
	dk_remove(${MSYS2}/var/lib/pacman/db.lck NOERROR)
	
	if(CLANG32)
		dk_msys2("pacman -S mingw-w64-clang-i686-autotools --needed --noconfirm")	# CLANG32
	elseif(CLANG64)
		dk_msys2("pacman -S mingw-w64-clang-x86_64-autotools --needed --noconfirm")	# CLANG64
	elseif(CLANGARM64)
		dk_msys2("pacman -S mingw-w64-clang-aarch64-autotools --needed --noconfirm")	# CLANGARM64
	elseif(MINGW32)
		dk_msys2("pacman -S mingw-w64-i686-autotools --needed --noconfirm")			# MINGW32
	elseif(MINGW64)
		dk_msys2("pacman -S mingw-w64-x86_64-autotools --needed --noconfirm")		# MINGW64
	elseif(UCRT64)
		dk_msys2("pacman -S mingw-w64-ucrt-x86_64-autotools --needed --noconfirm")	# UCRT64
	elseif(WIN_32)
		dk_msys2("pacman -S mingw-w64-i686-autotools --needed --noconfirm")			# WIN32 / MINGW32
	elseif(WIN_64)
		dk_msys2("pacman -S mingw-w64-x86_64-autotools --needed --noconfirm")		# WIN64	/ MINGW64
	endif()
endif()
