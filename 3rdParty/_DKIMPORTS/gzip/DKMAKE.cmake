include(${DKCMAKE_FUNCTIONS_DIR}/DK.cmake)
dk_load(dk_builder)
# https://git.savannah.gnu.org/cgit/gzip.git
# git://git.savannah.gnu.org/gzip.git
# https://github.com/kunpengcompute/gzip.git


### IMPORT ###
#dk_import(git://git.savannah.gnu.org/gzip.git)
#dk_import(https://github.com/kunpengcompute/gzip.git)
dk_import(https://github.com/kunpengcompute/gzip/archive/refs/heads/master.zip)


### LINK ###
#DEBUG_dk_include	(${GZIP}/${triple}/${DEBUG_DIR}/lib)
#RELEASE_dk_include	(${GZIP}/${triple}/${RELEASE_DIR}/lib)

# libversion
UNIX_dk_libDebug	(${GZIP}/${triple}/${DEBUG_DIR}/libver.a)
UNIX_dk_libRelease	(${GZIP}/${triple}/${RELEASE_DIR}/libver.a)
WIN_dk_libDebug		(${GZIP}/${triple}/${DEBUG_DIR}/ver.a)
WIN_dk_libRelease	(${GZIP}/${triple}/${RELEASE_DIR}/ver.a)

# libgzip
UNIX_dk_libDebug	(${GZIP}/${triple}/${DEBUG_DIR}/lib/libgzip.a)
UNIX_dk_libRelease	(${GZIP}/${triple}/${RELEASE_DIR}/lib/libgzip.a)
WIN_dk_libDebug		(${GZIP}/${triple}/${DEBUG_DIR}/lib/gzip.a)
WIN_dk_libRelease	(${GZIP}/${triple}/${RELEASE_DIR}/lib/gzip.a)


### 3RDPARTY LINK ###
DEBUG_dk_set		(GZIP_CMAKE -DGZIP_INCLUDE_DIR=${GZIP} -DGZIP_LIBRARY=${GZIP}/${triple}/${DEBUG_DIR}/lib/libgzip.a)
RELEASE_dk_set		(GZIP_CMAKE -DGZIP_INCLUDE_DIR=${GZIP} -DGZIP_LIBRARY=${GZIP}/${triple}/${RELEASE_DIR}/lib/libgzip.a)


### GENERATE / COMPILE ###
string(REPLACE "--disable-shared" 	"" 	GZIP_CONFIGURE "${DKCONFIGURE_BUILD}")
string(REPLACE "--enable-static" 	"" 	GZIP_CONFIGURE "${GZIP_CONFIGURE}")
string(REPLACE "  " 				" " GZIP_CONFIGURE "${GZIP_CONFIGURE}")

ANDROID_DEBUG_dk_cd			(${GZIP}/${triple}/${DEBUG_DIR})
ANDROID_DEBUG_dk_queueCommand		(${GZIP_CONFIGURE})

ANDROID_RELEASE_dk_cd			(${GZIP}/${triple}/${RELEASE_DIR})
ANDROID_RELEASE_dk_queueCommand		(${GZIP_CONFIGURE})

ANDROID_dk_build					(${GZIP})


APPLE_dk_cd					(${GZIP})
APPLE_dk_queueCommand				(chmod 777 configure)
APPLE_dk_queueCommand				(chmod 777 build-aux/git-version-gen)
APPLE_dk_queueCommand				(chmod 777 build-aux/install-sh)
APPLE_DEBUG_dk_cd				(${GZIP}/${triple}/${DEBUG_DIR})
APPLE_DEBUG_dk_queueCommand			(${GZIP_CONFIGURE} --disable-dependency-tracking "CFLAGS=-I${GZIP}/include -I${GZIP}/${triple}/${DEBUG_DIR}/lib")
#APPLE_DEBUG_dk_queueCommand		(make version)
#APPLE_DEBUG_dk_queueCommand		(make gzip)

APPLE_RELEASE_dk_cd			(${GZIP}/${triple}/${RELEASE_DIR})
APPLE_RELEASE_dk_queueCommand		(${GZIP_CONFIGURE} --disable-dependency-tracking "CFLAGS=-I${GZIP}/include -I${GZIP}/${triple}/${RELEASE_DIR}/lib")
#APPLE_RELEASE_dk_queueCommand		(make version)
#APPLE_RELEASE_dk_queueCommand		(make gzip)

APPLE_dk_build						(${GZIP})


EMSCRIPTEN_dk_cd				(${GZIP})
EMSCRIPTEN_dk_queueCommand			(chmod 777 configure)
EMSCRIPTEN_dk_queueCommand			(chmod 777 build-aux/git-version-gen)
EMSCRIPTEN_dk_queueCommand			(chmod 777 build-aux/install-sh)
EMSCRIPTEN_DEBUG_dk_cd			(${GZIP}/${triple}/${DEBUG_DIR})
EMSCRIPTEN_DEBUG_dk_queueCommand	(${GZIP_CONFIGURE})

EMSCRIPTEN_RELEASE_dk_cd		(${GZIP}/${triple}/${RELEASE_DIR})
EMSCRIPTEN_RELEASE_dk_queueCommand	(${GZIP_CONFIGURE})

EMSCRIPTEN_dk_build					(${GZIP})


LINUX_dk_cd					(${GZIP})
LINUX_dk_queueCommand				(chmod 777 configure)
LINUX_dk_queueCommand				(chmod 777 build-aux/git-version-gen)
LINUX_dk_queueCommand				(chmod 777 build-aux/install-sh)
LINUX_DEBUG_dk_cd				(${GZIP}/${triple}/${DEBUG_DIR})
LINUX_DEBUG_dk_queueCommand			(${GZIP_CONFIGURE})

LINUX_RELEASE_dk_cd			(${GZIP}/${triple}/${RELEASE_DIR})
LINUX_RELEASE_dk_queueCommand		(${GZIP_CONFIGURE})

LINUX_dk_build						(${GZIP})


RASPBERRY_dk_cd				(${GZIP})
RASPBERRY_dk_queueCommand			(chmod 777 configure)
RASPBERRY_dk_queueCommand			(chmod 777 build-aux/git-version-gen)
RASPBERRY_dk_queueCommand			(chmod 777 build-aux/install-sh)
RASPBERRY_DEBUG_dk_cd			(${GZIP}/${triple}/${DEBUG_DIR})
RASPBERRY_DEBUG_dk_queueCommand		(${GZIP_CONFIGURE})

RASPBERRY_RELEASE_dk_cd		(${GZIP}/${triple}/${RELEASE_DIR})
RASPBERRY_RELEASE_dk_queueCommand	(${GZIP_CONFIGURE})

RASPBERRY_dk_build					(${GZIP})


WIN_dk_cd						(${GZIP})
WIN_dk_queueCommand					(touch aclocal.m4 configure Makefile.am Makefile.in)
WIN_DEBUG_dk_cd				(${GZIP}/${triple}/${DEBUG_DIR})
WIN_DEBUG_dk_queueCommand			(${GZIP_CONFIGURE} --disable-dependency-tracking CFLAGS="-I${GZIP}/include -I${GZIP}/${triple}/${DEBUG_DIR}/lib")

WIN_RELEASE_dk_cd				(${GZIP}/${triple}/${RELEASE_DIR})
WIN_RELEASE_dk_queueCommand			(${GZIP_CONFIGURE} --disable-dependency-tracking CFLAGS="-I${GZIP}/include -I${GZIP}/${triple}/${RELEASE_DIR}/lib")

WIN_dk_build						(${GZIP} gzip)
