...
if(DEBUG)
	if(EMSCRIPTEN)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug linux-x32 -no-asm -static -no-sock -no-afalgeng -DOPENSSL_SYS_NETWARE -DSIG_DFL=0 -DSIG_IGN=0 -DHAVE_FORK=0 -DOPENSSL_NO_AFALGENG=1 -DOPENSSL_NO_SPEED=1)
	elseif(ANDROID_ARM32)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug android-arm -D__ANDROID_API__=${ANDROID_API})
	elseif(ANDROID_ARM64)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug CC=clang android-arm64 -D__ANDROID_API__=${ANDROID_API})
	elseif(IOS_ARM64)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug ios64-xcrun)
	elseif(IOSSIM)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug iossimulator-xcrun)
	elseif(LINUX_X86)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug linux-x32)
	elseif(LINUX_X86_64)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug linux-x86_64-clang)
	elseif(MAC)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug)
	elseif(RASPBERRY)
		dk_configure		(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug)
	elseif(WIN_X86)
		if(MSVC)
			dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --debug msvc CC=clang)
		else()
...
if(RELEASE)
	if(EMSCRIPTEN)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release linux-x32 -no-asm -static -no-sock -no-afalgeng -DOPENSSL_SYS_NETWARE -DSIG_DFL=0 -DSIG_IGN=0 -DHAVE_FORK=0 -DOPENSSL_NO_AFALGENG=1 -DOPENSSL_NO_SPEED=1)
	elseif(ANDROID_ARM32)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release android-arm -D__ANDROID_API__=${ANDROID_API})
	elseif(ANDROID_ARM64)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release android-arm64 -D__ANDROID_API__=${ANDROID_API})
	elseif(IOS_ARM64)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release ios64-xcrun)
	elseif(IOSSIM)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release iossimulator-xcrun)
	elseif(LINUX_X86)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release linux-x32)
	elseif(LINUX_X86_64)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release linux-x64)
	elseif(MAC)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release)
	elseif(RASPBERRY)
		dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release)
	elseif(WIN_X86)
		if(MSVC)
			dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release msvc CC=clang)
		else()
			dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release mingw CC=clang)
		endif()
	elseif(WIN_X86_64)
		if(MSVC)
			dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release msvc64 CC=clang)
		else()
			dk_configure	(${OPENSSL_DIR} ${PERL_EXE} ../../Configure no-shared --release mingw64 CC=clang)
		endif()
	endif()
endif()
