...
function(AliasFunctions name)
	...
	CreateFunc("function(IOS_${name})\n if(IOS)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOS32_${name})\n if(IOS_32)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOS64_${name})\n if(IOS_64)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM_${name})\n if(IOSSIM)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM32_${name})\n if(IOSSIM_32)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM64_${name})\n if(IOSSIM_64)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX_${name})\n if(LINUX)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX32_${name})\n if(LINUX_32)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX64_${name})\n if(LINUX_64)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(RASPBERRY_${name})\n if(RASPBERRY)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(RASPBERRY32_${name})\n if(RASPBERRY_32)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(RASPBERRY64_${name})\n if(RASPBERRY_64)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(ANDROID_${name})\n if(ANDROID)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	...
	CreateFunc("function(IOS_DEBUG_${name})\n if(IOS AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOS32_DEBUG_${name})\n if(IOS_32 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOS64_DEBUG_${name})\n if(IOS_64 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM_DEBUG_${name})\n if(IOSSIM AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM32_DEBUG_${name})\n if(IOSSIM_32 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM64_DEBUG_${name})\n if(IOSSIM_64 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX_DEBUG_${name})\n if(LINUX AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX32_DEBUG_${name})\n if(LINUX_32 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX64_DEBUG_${name})\n if(LINUX_64 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(RASPBERRY_DEBUG_${name})\n if(RASPBERRY AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(RASPBERRY32_DEBUG_${name})\n if(RASPBERRY_32 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(RASPBERRY64_DEBUG_${name})\n if(RASPBERRY_64 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(ANDROID_DEBUG_${name})\n if(ANDROID AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(ANDROID32_DEBUG_${name})\n if(ANDROID_32 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(ANDROID64_DEBUG_${name})\n if(ANDROID_64 AND DEBUG)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	...
	CreateFunc("function(IOS_RELEASE_${name})\n if(IOS AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOS32_RELEASE_${name})\n if(IOS_32 AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOS64_RELEASE_${name})\n if(IOS_64 AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM_RELEASE_${name})\n if(IOSSIM AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM32_RELEASE_${name})\n if(IOSSIM_32 AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(IOSSIM64_RELEASE_${name})\n if(IOSSIM_64 AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX_RELEASE_${name})\n if(LINUX AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX32_RELEASE_${name})\n if(LINUX_32 AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(LINUX64_RELEASE_${name})\n if(LINUX_64 AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(RASPBERRY_RELEASE_${name})\n if(RASPBERRY AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
	CreateFunc("function(RASPBERRY32_RELEASE_${name})\n if(RASPBERRY_32 AND RELEASE)\n ${name}(\${ARGV})\n endif()\n endfunction()\n")
...
function(DKEXTRACT src dest)
	...
	if(NOT EXISTS ${CMAKE_EXE})
		DKERROR("CMAKE_EXE not found: \${CMAKE_EXE} = ${CMAKE_EXE}")
	endif()
...
function(DKINSTALL src_path import_path dest_path)
	...
	if(NOT ${import_path} STREQUAL ${import_path_lower})
		DKERROR("ERROR: 2nd parameter in DKINSTALL() (${import_path}) must be all lowercase")
	endif()
	string(FIND ${import_path} - index)
	if(${import_path} GREATER -1)
		DKERROR("ERROR: 2nd parameter in DKINSTALL() (${import_path}) contains a - , please convert it to an underscore _")
	endif()
	
	if(NOT EXISTS ${DKIMPORTS}/${import_path})
		DKERROR("ERROR: 2nd parameter in DKINSTALL() (${DKIMPORTS}/${import_path}) does not exist")
	endif()
	if(EXISTS ${dest_path}/installed)
		#DKINFO("${import_path} installed")
		return()
	endif()
	DKINFO("\n")
	DKINFO("DKINSTALL((): src_path = ${src_path}")
	dk_getDirectory(${src_path} src_directory)
	DKINFO("DKINSTALL((): src_directory = ${src_directory}")
	dk_getFilename(${src_path} src_filename)
	DKINFO("DKINSTALL((): src_filename = ${src_filename}")
	dk_getExtension(${src_filename} src_extension)
	DKINFO("DKINSTALL((): src_extension = ${src_extension}")
	DKINFO("\n")
	DKINFO("DKINSTALL((): dest_path = ${dest_path}")
	dk_getDirectory(${dest_path} dest_directory)
	DKINFO("DKINSTALL((): dest_directory = ${dest_directory}")
	dk_getFilename(${dest_path} dest_filename)
	DKINFO("DKINSTALL((): dest_filename = ${dest_filename}")
	dk_getExtension(${dest_filename} dest_extension)
	DKINFO("DKINSTALL((): dest_extension = ${dest_extension}")
	DKINFO("\n")
	...
	DKINFO("The Downloaded file ${${dl_filename}} is a ${FILETYPE} file ${src_extension}")
	if(${FILETYPE} STREQUAL "UNKNOWN")
		DKSET(FILETYPE "Archive")
		DKINFO("We will try to extract it in case it's an archive, but it may fail.")
	endif()
	...
	if("${ARGN}" STREQUAL "NOPATCH")
		...
		DKINFO("COPYING PATCH FILES FROM _IMPORTS/${import_path} TO COMPLETE INSTALL.")
		DKINFO("To stop patch files from overwrites install files, add a \"NOPATCH\" argument to the end of the DKINSTALL command")
