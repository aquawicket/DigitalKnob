...
FOREACH(plugin ${DKPLUGS})
	...
	IF(${_index} GREATER -1)
		...
		RASPBERRY_DEBUG_LIB(${PATHTOPLUGIN}/${OS}/Debug/obj/lib${plugin}.a)
		RASPBERRY_RELEASE_LIB(${PATHTOPLUGIN}/${OS}/Release/obj/lib${plugin}.a)
	...
	IF(QUEUE_BUILD AND CMAKE_FILE)
		...
		IF(RASPBERRY_32)
			...
			IF(DEBUG)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/${OS}/Debug)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				RASPBERRY32_COMMAND(${CMAKE_COMMAND} -G "Unix Makefiles" -DLINUX_32=ON -DREBUILD=ON -DDEBUG=ON ${PATHTOPLUGIN})
				RASPBERRY32_COMMAND(make)
			ENDIF()
			IF(RELEASE)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/${OS}/Release)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				RASPBERRY32_COMMAND(${CMAKE_COMMAND} -G "Unix Makefiles" -DLINUX_32=ON -DREBUILD=ON -DRELEASE=ON ${PATHTOPLUGIN})
				RASPBERRY32_COMMAND(make)
			ENDIF()
		ENDIF()
		IF(RASPBERRY_64)
			...
			IF(DEBUG)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/${OS}/Debug)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				RASPBERRY64_COMMAND(${CMAKE_COMMAND} -G "Unix Makefiles" -DLINUX_64=ON -DREBUILD=ON -DDEBUG=ON ${PATHTOPLUGIN})
				RASPBERRY64_COMMAND(make)
			ENDIF()
			IF(RELEASE)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/${OS}/Release)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				RASPBERRY64_COMMAND(${CMAKE_COMMAND} -G "Unix Makefiles" -DLINUX_64=ON -DREBUILD=ON -DRELEASE=ON ${PATHTOPLUGIN})
				RASPBERRY64_COMMAND(make)
			ENDIF()
		ENDIF()
