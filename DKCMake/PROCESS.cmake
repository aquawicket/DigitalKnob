...
DKSET(DKINCLUDES "")
DKSET(DKLIBRARIES "")
ANDROID_RELEASE_LIB(C:/android-ndk-r10d/sources/cxx-stl/gnu-libstdc++/4.8/libs/armeabi-v7a/libgnustl_static.a)
FOREACH(plugin ${DKPLUGS})
	...
	IF(${_index} GREATER -1)
		...
		ANDROID_DEBUG_LIB(${PATHTOPLUGIN}/${OS}/obj/local/armeabi-v7a/lib${plugin}.a)
		ANDROID_RELEASE_LIB(${PATHTOPLUGIN}/${OS}/obj/local/armeabi-v7a/lib${plugin}.a)
	...
	IF(QUEUE_BUILD AND CMAKE_FILE)
		...
		IF(NOT EXISTS ${PATHTOPLUGIN}/CMakeLists.txt)
			...
			IF(WIN_32)
				...
				WIN32_COMMAND(${CMAKE_COMMAND} -G ${GENERATOR} -DWIN_32=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${PATHTOPLUGIN})
				IF(DEBUG)
			...
			IF(WIN_64)
				...
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				WIN64_COMMAND(${CMAKE_COMMAND} -G ${X64GENERATOR} -DWIN_64=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${PATHTOPLUGIN})
				IF(DEBUG)
			...
			IF(IOS_32)
				...
				IF(DEBUG)
					IOS32_COMMAND(xcodebuild -configuration Debug build -arch i386 -sdk iphonesimulator6.1)
				ENDIF()
				IF(RELEASE)
					IOS32_COMMAND(xcodebuild -configuration Release build -arch i386 -sdk iphonesimulator6.1)
				ENDIF()
			ENDIF()
			IF(IOS_64)
				...
				IF(DEBUG)
					IOS64_COMMAND(xcodebuild -configuration Debug build -arch x86_64 -sdk iphonesimulator6.1)
				ENDIF()
				IF(RELEASE)
					IOS64_COMMAND(xcodebuild -configuration Release build -arch x86_64 -sdk iphonesimulator6.1)
				ENDIF()
			ENDIF()
			IF(IOSSIM_32)
				...
				IF(DEBUG)
					IOSSIM32_COMMAND(xcodebuild -configuration Debug build -arch i386 -sdk iphonesimulator6.1)
				ENDIF()
				IF(RELEASE)
					IOSSIM32_COMMAND(xcodebuild -configuration Release build -arch i386 -sdk iphonesimulator6.1)
				ENDIF()
			ENDIF()
			IF(IOSSIM_64)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/iossim64)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				IOSSIM64_COMMAND(${CMAKE_COMMAND} -G "Xcode" -DIOSSIM_64=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON -DCMAKE_TOOLCHAIN_FILE=${DKCMAKE}/iOS.cmake -DIOS_PLATFORM=SIMULATOR ${PATHTOPLUGIN})
				IF(DEBUG)
					IOSSIM64_COMMAND(xcodebuild -configuration Debug build -arch x86_64 -sdk iphonesimulator6.1)
				ENDIF()
				IF(RELEASE)
					IOSSIM64_COMMAND(xcodebuild -configuration Release build -arch x86_64 -sdk iphonesimulator6.1)
				ENDIF()
			ENDIF()
			IF(LINUX_32)
	...
	IF(ANDROID_LIBMK)
		...
		MESSAGE("Creating Application.mk file for ${plugin}....")
...
IF(WIN_32)
	...
	LIST(APPEND DEBUG_LINK_FLAGS /level='requireAdministrator')
	LIST(APPEND DEBUG_LINK_FLAGS /uiAccess='false')
	...
	LIST(APPEND RELEASE_LINK_FLAGS /level='requireAdministrator')
	LIST(APPEND RELEASE_LINK_FLAGS /uiAccess='false')
...
IF(MAC)
	...
	IF(DEBUG)
		DKREMOVE(${DKPROJECT}/assets/USER)
		DKCOPY(${DKPROJECT}/assets/ ${DKPROJECT}/${OS}/Debug/${AppName}.app/Contents/MacOS TRUE)
	...
	IF(RELEASE)
		DKREMOVE(${DKPROJECT}/assets/USER)
		DKCOPY(${DKPROJECT}/assets/ ${DKPROJECT}/${OS}/Release/${AppName}.app/Contents/MacOS TRUE)
...
IF(IOS)
	...
	DKREMOVE(${DKPROJECT}/assets/USER)
	IF(DEBUG)
		DKCOPY(${DKPROJECT}/assets/ ${DKPROJECT}/${OS}/${DEBUG}/${AppName}.app TRUE)
		DKCOPY(${DKPROJECT}/icons/ios/ ${DKPROJECT}/${OS}/${DEBUG}/${AppName}.app TRUE)
	ENDIF()
	IF(RELEASE)
		DKCOPY(${DKPROJECT}/assets/ ${DKPROJECT}/${OS}/${RELEASE}/${AppName}.app TRUE)
...
IF(LINUX)
	...
	FIND_LIBRARY(STDLIB STDC++) ## Do we Append this?
	...
	SET(CMAKE_CXX_LINK_EXECUTABLE "${CMAKE_CXX_LINK_EXECUTABLE} -ldl -lXrandr -lX11")
...
IF(ANDROID)
	...
	DKSET(ANDROID_APPMK ${ANDROID_APPMK} "NDKLIBDIR := C:/android-ndk-r10d/sources/cxx-stl/gnu-libstdc++/4.8/libs/armeabi-v7a\n")
