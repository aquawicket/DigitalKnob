...
DKSET(DKINCLUDES "")
DKSET(DKLIBRARIES "")
ANDROID_DEBUG_LIB(${3RDPARTY}/android-ndk-r20b/sources/cxx-stl/gnu-libstdc++/4.8/libs/armeabi-v7a/libgnustl_static.a)
ANDROID_RELEASE_LIB(${3RDPARTY}/android-ndk-r20b/sources/cxx-stl/gnu-libstdc++/4.8/libs/armeabi-v7a/libgnustl_static.a)
FOREACH(plugin ${DKPLUGS})
	...
	IF(QUEUE_BUILD AND CMAKE_FILE)
		...
		IF(ANDROID_32)
			FILE(MAKE_DIRECTORY ${PATHTOPLUGIN}/android32)
			IF(DEBUG)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/${OS}/Debug)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				ANDROID32_COMMAND(${CMAKE_COMMAND} -G ${GENERATOR} -A ARM -DANDROID_32=ON -DREBUILD=ON -DDEBUG=ON -DCMAKE_TOOLCHAIN_FILE=C:\Microsoft\AndroidNDK64\android-ndk-r20b\build\cmake\android.toolchain.cmake -DANDROID_NDK=C:\Microsoft\AndroidNDK64\android-ndk-r20b ${PATHTOPLUGIN})
				ANDROID32_COMMAND(make)
			ENDIF()
			IF(RELEASE)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/${OS}/Release)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				ANDROID32_COMMAND(${CMAKE_COMMAND} -G ${GENERATOR} -A ARM -DANDROID_32=ON -DREBUILD=ON -DRELEASE=ON -DCMAKE_TOOLCHAIN_FILE=C:\Microsoft\AndroidNDK64\android-ndk-r20b\build\cmake\android.toolchain.cmake -DANDROID_NDK=C:\Microsoft\AndroidNDK64\android-ndk-r20b ${PATHTOPLUGIN})
				ANDROID32_COMMAND(make)
			ENDIF()
		ENDIF()
		...
		IF(ANDROID_64)
			...
			IF(DEBUG)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/${OS}/Debug)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				ANDROID64_COMMAND(${CMAKE_COMMAND} -G ${GENERATOR} -A ARM64 -DANDROID_64=ON -DREBUILD=ON -DDEBUG=ON -DCMAKE_TOOLCHAIN_FILE=C:\Microsoft\AndroidNDK64\android-ndk-r20b\build\cmake\android.toolchain.cmake -DANDROID_NDK=C:\Microsoft\AndroidNDK64\android-ndk-r20b ${PATHTOPLUGIN})
				ANDROID64_COMMAND(make)
			ENDIF()
			IF(RELEASE)
				DKSET(CURRENT_DIR ${PATHTOPLUGIN}/${OS}/Release)
				FILE(MAKE_DIRECTORY ${CURRENT_DIR})
				ANDROID64_COMMAND(${CMAKE_COMMAND} -G ${GENERATOR} -A ARM64 -DANDROID_32=ON -DREBUILD=ON -DRELEASE=ON -DCMAKE_TOOLCHAIN_FILE=C:\Microsoft\AndroidNDK64\android-ndk-r20b\build\cmake\android.toolchain.cmake -DANDROID_NDK=C:\Microsoft\AndroidNDK64\android-ndk-r20b ${PATHTOPLUGIN})
				ANDROID64_COMMAND(make)
			ENDIF()
		ENDIF()
	...
	IF(ANDROID_LIBMK)
		...
		IF(DEBUG)
			...
			IF(NOT CMAKE_HOST_UNIX)
				...
				##EXECUTE_PROCESS(COMMAND ${3RDPARTY}/android-ndk-r20b/ndk-build.cmd NDK_DEBUG=1 WORKING_DIRECTORY ${CURRENT_DIR})
			ENDIF()
			IF(CMAKE_HOST_UNIX)
				EXECUTE_PROCESS(COMMAND ${ANDROID_NDK}/ndk-build NDK_DEBUG=1 WORKING_DIRECTORY ${CURRENT_DIR})
				##EXECUTE_PROCESS(COMMAND ${3RDPARTY}/android-ndk-r20b/ndk-build NDK_DEBUG=1 WORKING_DIRECTORY ${CURRENT_DIR})
			ENDIF()
		ENDIF()
		IF(RELEASE)
			...
			IF(NOT CMAKE_HOST_UNIX)
				...
				##EXECUTE_PROCESS(COMMAND ${3RDPARTY}/android-ndk-r20b/ndk-build.cmd NDK_DEBUG=0 WORKING_DIRECTORY ${CURRENT_DIR})
			ENDIF()
			IF(CMAKE_HOST_UNIX)
				EXECUTE_PROCESS(COMMAND ${ANDROID_NDK}/ndk-build NDK_DEBUG=0 WORKING_DIRECTORY ${CURRENT_DIR})
				##EXECUTE_PROCESS(COMMAND ${3RDPARTY}/android-ndk-r20b/ndk-build NDK_DEBUG=0 WORKING_DIRECTORY ${CURRENT_DIR})
			ENDIF()
		ENDIF()
	ENDIF()
...
IF(ANDROID)
	...
	##DKSET(ANDROID_APPMK ${ANDROID_APPMK} "NDKLIBDIR := ${3RDPARTY}/android-ndk-r20b/sources/cxx-stl/gnu-libstdc++/4.8/libs/armeabi-v7a\n")
