...
foreach(plugin ${dkdepend_list})
	...
	if(${isDKPlugin} GREATER -1)
		...
		if(PREBUILD)
			...
			if(WIN_32)
				WIN_DKCOMMAND(${DKCMAKE_BUILD} -DWIN_32=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					WIN32_DKCOMMAND(${MSBUILD} ${CURRENT_DIR}/${plugin}.sln /p:Configuration=Debug)
				endif()
				if(RELEASE)
					WIN32_DKCOMMAND(${MSBUILD} ${CURRENT_DIR}/${plugin}.sln /p:Configuration=Release)
				endif()
			endif()
			if(WIN_64)
				WIN_DKCOMMAND(${DKCMAKE_BUILD} -DWIN_64=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					WIN64_DKCOMMAND(${MSBUILD} ${CURRENT_DIR}/${plugin}.sln /p:Configuration=Debug)
				endif()
				if(RELEASE)
					WIN64_DKCOMMAND(${MSBUILD} ${CURRENT_DIR}/${plugin}.sln /p:Configuration=Release)
				endif()
			endif()
			if(MAC_32)
				MAC_DKCOMMAND(${DKCMAKE_BUILD} -DMAC_32=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					MAC32_DKCOMMAND(xcodebuild -configuration Debug build)
				endif()
				if(RELEASE)
					MAC32_DKCOMMAND(xcodebuild -configuration Release build)
				endif()
			endif()
			if(MAC_64)
				MAC_DKCOMMAND(${DKCMAKE_BUILD} -DMAC_64=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					MAC_DKCOMMAND(xcodebuild -configuration Debug build)
				endif()
				if(RELEASE)
					MAC_DKCOMMAND(xcodebuild -configuration Release build)
				endif()
			endif()
			if(IOS_32)
				IOS_DKCOMMAND(${DKCMAKE_BUILD} -DIOS_32=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					IOS_DKCOMMAND(xcodebuild -configuration Debug build) #-sdk iphoneos13.0)
				endif()
				if(RELEASE)
					IOS_DKCOMMAND(xcodebuild -configuration Release build)
				endif()
			endif()
			if(IOS_64)
				IOS_DKCOMMAND(${DKCMAKE_BUILD} -DIOS_64=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					IOS_DKCOMMAND(xcodebuild -configuration Debug build)
				endif()
				if(RELEASE)
					IOS_DKCOMMAND(xcodebuild -configuration Release build)
				endif()
			...
			if(IOSSIM_32)
				IOSSIM_DKCOMMAND(${DKCMAKE_BUILD} -DIOSSIM_32=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					IOSSIM_DKCOMMAND(xcodebuild -configuration Debug build)
				endif()
				if(RELEASE)
					IOSSIM_DKCOMMAND(xcodebuild -configuration Release build)
				endif()
			endif()
			if(IOSSIM_64)
				IOSSIM_DKCOMMAND(${DKCMAKE_BUILD} -DIOSSIM_64=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					IOSSIM_DKCOMMAND(xcodebuild -configuration Debug build)
				endif()
				if(RELEASE)
			...
			if(LINUX_32)
				if(DEBUG)
					LINUX_DKCOMMAND(${DKCMAKE_BUILD} -DLINUX_32=ON -DDEBUG=ON -DREBUILD=ON ${plugin_path})
					LINUX_DKCOMMAND(make)
				elseif(RELEASE)
					LINUX_DKCOMMAND(${DKCMAKE_BUILD} -DLINUX_32=ON -DREBUILD=ON -DRELEASE=ON ${plugin_path})
					LINUX_DKCOMMAND(make)
				endif()
			endif()
			if(LINUX_64)
				if(DEBUG)
					LINUX_DKCOMMAND(${DKCMAKE_BUILD} -DLINUX_64=ON -DDEBUG=ON -DREBUILD=ON ${plugin_path})
					LINUX_DKCOMMAND(make)
				elseif(RELEASE)
					LINUX_DKCOMMAND(${DKCMAKE_BUILD} -DLINUX_64=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
					LINUX_DKCOMMAND(make)
				endif()
			endif()
			if(RASPBERRY_32)
				if(DEBUG)
					RASPBERRY_DKCOMMAND(${DKCMAKE_BUILD} -DRASPBERRY_32=ON -DDEBUG=ON -DREBUILD=ON ${plugin_path})
					RASPBERRY_DKCOMMAND(make)
				elseif(RELEASE)
					RASPBERRY_DKCOMMAND(${DKCMAKE_BUILD} -DRASPBERRY_32=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
					RASPBERRY_DKCOMMAND(make)
				endif()
			endif()
			if(RASPBERRY_64)
				if(DEBUG)
					RASPBERRY_DKCOMMAND(${DKCMAKE_BUILD} -DRASPBERRY_64=ON -DDEBUG=ON -DREBUILD=ON ${plugin_path})
					RASPBERRY_DKCOMMAND(make)
				elseif(RELEASE)
					RASPBERRY_DKCOMMAND(${DKCMAKE_BUILD} -DRASPBERRY_64=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
					RASPBERRY_DKCOMMAND(make)
				endif()
			endif()
			if(ANDROID_32)
				ANDROID_DKCOMMAND(${DKCMAKE_BUILD} -DANDROID_32=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					ANDROID32_DKCOMMAND(${MSBUILD} ${CURRENT_DIR}/${plugin}.sln /p:Configuration=Debug)
				endif()
				if(RELEASE)
					ANDROID32_DKCOMMAND(${MSBUILD} ${CURRENT_DIR}/${plugin}.sln /p:Configuration=Release)
				endif()
			endif()
			if(ANDROID_64)
				ANDROID_DKCOMMAND(${DKCMAKE_BUILD} -DANDROID_64=ON -DDEBUG=ON -DRELEASE=ON -DREBUILD=ON ${plugin_path})
				if(DEBUG)
					ANDROID64_DKCOMMAND(${MSBUILD} ${CURRENT_DIR}/${plugin}.sln /p:Configuration=Debug)
				endif()
				if(RELEASE)
					ANDROID64_DKCOMMAND(${MSBUILD} ${CURRENT_DIR}/${plugin}.sln /p:Configuration=Release)
				endif()
			endif()
