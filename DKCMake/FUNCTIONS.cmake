...
function(DKCOPY from to overwrite)
	if(EXISTS ${from})
		...
		message(WARNING "DKCOPY(): The source path does not exist. (${from})")
	endif()
endfunction()
...
function(DKINSTALL url import_path destination_path)
	...
	if(index GREATER -1)
		...
		message(FATAL_ERROR "DKINSTALL(${import_path}): 3rd parameter requires a full path")
...
function(WIN_BASH arg)
	if(WIN AND QUEUE_BUILD)
		...
		message(STATUS "BASH -> ${arg}")
		DKEXECUTE_PROCESS(COMMAND cmd /c ${MSYS}/bin/bash ${MSYS}/dkscript.tmp WORKING_DIRECTORY ${MSYS})
	endif()
endfunction()
...
function(ANDROID_NDK_DEBUG arg)
	if(ANDROID AND DEBUG AND QUEUE_BUILD)
		if(CMAKE_HOST_WIN32)
			DKEXECUTE_PROCESS(COMMAND ${ANDROIDNDK}/ndk-build.cmd WORKING_DIRECTORY ${3RDPARTY}/${arg}/${OS}/Debug)
		endif()
		if(CMAKE_HOST_UNIX)
			DKEXECUTE_PROCESS(COMMAND ${ANDROIDNDK}/ndk-build WORKING_DIRECTORY ${3RDPARTY}/${arg}/${OS}/Debug)
		endif()
	endif()
endfunction()
...
function(ANDROID_NDK_RELEASE arg)
	if(ANDROID AND RELEASE AND QUEUE_BUILD)
		if(CMAKE_HOST_WIN32)
			DKEXECUTE_PROCESS(COMMAND ${ANDROIDNDK}/ndk-build.cmd WORKING_DIRECTORY ${3RDPARTY}/${arg}/${OS}/Release)
		endif()
		if(CMAKE_HOST_UNIX)
			DKEXECUTE_PROCESS(COMMAND ${ANDROIDNDK}/ndk-build WORKING_DIRECTORY ${3RDPARTY}/${arg}/${OS}/Release)
		endif()
	endif()
endfunction()
...
function(DKPLUGIN arg)
	...
	if(ANDROID_TESTTESTTEST)
		...
		if(ANDROID)
			...
			DKSET(CMAKE_FILE "${CMAKE_FILE}set(CMAKE_CXX_FLAGS \"-std=c++1z\")\n") #c++17
			DKSET(CMAKE_FILE "${CMAKE_FILE}set(CMAKE_CXX_DEBUG_FLAGS \"-g2 -gdwarf-2 -O0\")\n")
			DKSET(CMAKE_FILE "${CMAKE_FILE}set(CMAKE_CXX_RELEASE_FLAGS \"-O3\")\n")
			DKSET(CMAKE_FILE "${CMAKE_FILE}set(CMAKE_ANDROID_STL_TYPE c++_static)\n")			
		endif()
...
function(DKASSETS arg)
	...
	message(STATUS "Importing assets...")
...
function(DKSETPATHTOPLUGIN arg)
	...
	if(EXISTS ${DKPLUGINS}/${arg}/DKMAKE.cmake)
		...
		message(STATUS "FOUND ${arg} DKPlugin at ${PATHTOPLUGIN}")
		return()
	elseif(EXISTS ${DKIMPORTS}/${arg}/DKMAKE.cmake)
		DKSET(PATHTOPLUGIN "${DKIMPORTS}/${arg}")
		message(STATUS "FOUND ${arg} DKImport at ${PATHTOPLUGIN}")
		return()
	endif()
	...
	file(GLOB children RELATIVE ${DIGITALKNOB}/../ ${DIGITALKNOB}/../*)
  	foreach(child ${children})
		if(EXISTS ${DIGITALKNOB}/../${child}/DKPlugins/${arg}/DKMAKE.cmake)
			...
			message(STATUS "FOUND ${arg} Plugin at ${PATHTOPLUGIN}")
			return()
    	endif()
  	endforeach()
...
function(DKDEPEND arg)
	...
	if(${_index} GREATER -1)
		message(STATUS "${arg} IS DISABLED")
		return()
	endif()
		...
		foreach(each_file ${allfiles})
			...
			if(NOT IS_DIRECTORY ${filepath})
				...
				if(${indexD} GREATER -1)
					...
					if(${index} GREATER -1)
						message(STATUS "Replacing 'dkapp' with '${arg}' in ${filepath}")
...
function(DKUPDATE_INFO_PLIST arg)
	...
	if(IOS OR IOSSIM)
		...
		message(STATUS "CHECKING FOR... ${DKPROJECT}/${OS}/${DEBUG_DIR}/${arg}.app/Info.plist")
