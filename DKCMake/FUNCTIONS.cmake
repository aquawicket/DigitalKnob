...
function(DKUPDATE_ANDROID_NAME arg)
	...
	if(ANDROID)
		...
		foreach(each_file ${allfiles})
			...
			if(NOT IS_DIRECTORY ${filepath})
				...
				if(${indexA} GREATER -1)
					message("\n Replacing 'dkapp' with '${arg}' in ${filepath} \n")
					string(REPLACE "dkapp" "${arg}" filestring "${filestring}")
					file(WRITE ${filepath} "${filestring}")
				endif()
			endif()
			
			string(FIND "${each_file}" "dkapp" indexB)
			if(${indexB} GREATER -1)	
				string(FIND "${each_file}" "/" indexC)
				if(${indexC} EQUAL -1)
					set(new_name ${each_file})
					string(REPLACE "dkapp" "${arg}" new_name "${new_name}")
					message("\n Renaming ${each_file} to ${new_name} \n")
					DKRENAME(${DKPROJECT}/${OS}/${each_file} ${DKPROJECT}/${OS}/${new_name})
				endif()
			endif()
		endforeach()
		
		##DKREMOVE(${DKPROJECT}/${OS}/${RELEASE_DIR}/src/digitalknob/${arg})
		##DKRENAME(${DKPROJECT}/${OS}/${RELEASE_DIR}/src/digitalknob/dkapp ${DKPROJECT}/${OS}/${RELEASE_DIR}/src/digitalknob/${arg})
	endif()
	...
	#	file(GLOB_RECURSE allfiles RELATIVE "${DKPROJECT}/${OS}/${DEBUG_DIR}/" "${DKPROJECT}/${OS}/${DEBUG_DIR}/*.xml" "${DKPROJECT}/${OS}/${DEBUG_DIR}/*.java")
	#	foreach(each_file ${allfiles})
	#		set(thefile "${DKPROJECT}/${OS}/${DEBUG_DIR}/${each_file}")
	#		file(READ ${thefile} filestring)
	#		string(REPLACE "dkapp" "${arg}" filestring "${filestring}")
	#		file(WRITE ${thefile} "${filestring}")
	#
	#	
	#	DKREMOVE(${DKPROJECT}/${OS}/${DEBUG_DIR}/src/digitalknob/${arg})
	#	DKRENAME(${DKPROJECT}/${OS}/${DEBUG_DIR}/src/digitalknob/dkapp ${DKPROJECT}/${OS}/${DEBUG_DIR}/src/digitalknob/${arg})
	#endif()
	#if(ANDROID AND RELEASE)
	#	## update all .xml and .java files recursivley
	#	file(GLOB_RECURSE allfiles RELATIVE "${DKPROJECT}/${OS}/${RELEASE_DIR}/" "${DKPROJECT}/${OS}/${RELEASE_DIR}/*.xml" "${DKPROJECT}/${OS}/${RELEASE_DIR}/*.java")
	#	foreach(each_file ${allfiles})
	#		set(thefile "${DKPROJECT}/${OS}/${RELEASE_DIR}/${each_file}")
	#		message(${thefile})
	#		file(READ ${thefile} filestring)
	#		string(REPLACE "dkapp" "${arg}" filestring "${filestring}")
	#		file(WRITE ${thefile} "${filestring}")
