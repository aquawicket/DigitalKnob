...
function(Wait)
	if(CMAKE_HOST_WIN32)
		##Don't use DKEXECUTE_PROCESS here. It will fail.
		execute_process(COMMAND cmd /c echo press and key to continue && timeout /t 60 > nul WORKING_DIRECTORY ${CMAKE_SOURCE_DIR})
		return()
	endif()
	if(CMAKE_HOST_LINUX)
		DKEXECUTE_PROCESS(read -n 1 -s -r -p "Press any key to continue" WORKING_DIRECTORY ${CMAKE_SOURCE_DIR})
		return()
	endif()	
...
function(DELETE_CACHE)
	...
	if(CMAKE_HOST_WIN32)
		DKEXECUTE_PROCESS(for /r %i in (CMakeCache.*) do del "%i" WORKING_DIRECTORY ${DK})
        DKEXECUTE_PROCESS(for /d /r %i in (*CMakeFiles*) do rmdir /s /Q "%i" WORKING_DIRECTORY ${DK})
	else()
		DKEXECUTE_PROCESS(find . -name "CMakeCache.*" -delete WORKING_DIRECTORY ${DK})
		DKEXECUTE_PROCESS(find . -type d -name "CMakeFiles" -delete WORKING_DIRECTORY ${DK})
	endif()
endfunction()
...
function(DKDOWNLOAD url) #arg2 destination_path
	...
	if(NOT EXISTS ${CURRENT_DIR}/${filename})
		...
		if(NOT status_code EQUAL 0)
			...
			##WIN_COMMAND("${DK}/DKCMake/DKCurl.exe ${url} ${CURRENT_DIR}/${filename}")
		else()
...
function(DKINSTALL url import_path destination_path)
	...
	if(${FILETYPE} STREQUAL "Archive")
		DKREMOVE(${DK}/Download/UNZIPPED)
		DKEXTRACT(${DK}/Download/${filename} ${DK}/Download/UNZIPPED)
		#We either have a root folder in /UNZIPPED, or multiple files without a root folder
		file(GLOB items RELATIVE "${DK}/Download/UNZIPPED/" "${DK}/Download/UNZIPPED/*")
		list(LENGTH items count)
		if(${count} GREATER 2) ##NOTE: This should be "${count} GREATER 1" but msys has a readme file in it next to the inner msys folder and that messes things up for more than 1
			...
			DKRENAME(${DK}/Download/UNZIPPED ${destination_path})
		else()
			if(EXISTS ${DK}/Download/UNZIPPED/${folder}) ##Zip extracted to expected folder. Move the folder to 3rdParty
				DKRENAME(${DK}/Download/UNZIPPED/${folder} ${destination_path})
				DKREMOVE(${DK}/Download/UNZIPPED)
			else() #Zip extracted to a root folder, but not named what we expected. Rename and move folder to 3rdParty
				foreach(item ${items})
					if(NOT IS_DIRECTORY ${DK}/Download/UNZIPPED/${item})
						list(REMOVE_ITEM items ${item}) #remove any readme.txt or other non-directory items
					endif()
				endforeach()
				DKRENAME(${DK}/Download/UNZIPPED/${items} ${destination_path})
				DKREMOVE(${DK}/Download/UNZIPPED)
			endif() 
		endif()
	...
	#	DKSET(QUEUE_BUILD ON)
	#	DKEXECUTE(${DK}/Download/${filename})
	else() #NOT ARCHIVE, just copy the file into it's 3rdParty folder
		DKCOPY(${DK}/Download/${filename} ${destination_path}/${filename} TRUE)
	endif()
...
function(dk_getShortPath path result)
	if(CMAKE_HOST_WIN32)
		execute_process(COMMAND ${DKCMAKE}/getShortPath.cmd ${path} OUTPUT_VARIABLE path WORKING_DIRECTORY ${DK})
...
function(DKEXECUTE_PROCESS fullcmnd)
	...
	if(NOT ${result} EQUAL 0)
		if(CMAKE_HOST_WIN32)
			execute_process(COMMAND timeout /t 2 /nobreak OUTPUT_QUIET WORKING_DIRECTORY ${DK}) ##wait 2 seconds for the stdout to flush before printing error
		else()
			execute_process(COMMAND sleep 2 WORKING_DIRECTORY ${DK}) ##wait for unix
		endif()
...
function(WIN32_DEBUG_COMMAND args)
...
function(WIN32_RELEASE_COMMAND args)
...
function(WIN64_DEBUG_COMMAND args)
...
function(WIN64_RELEASE_COMMAND args)
...
function(MAC_DEBUG_COMMAND args)
...
function(MAC_RELEASE_COMMAND args)
...
function(MAC32_DEBUG_COMMAND args)
...
function(MAC32_RELEASE_COMMAND args)
...
function(MAC64_DEBUG_COMMAND args)
...
function(MAC64_RELEASE_COMMAND args)
...
function(IOS_DEBUG_COMMAND args)
	if(IOS AND DEBUG)
		set(args ${args} ${ARGN})
		IOS_COMMAND(${args})
	endif()
...
function(IOS_RELEASE_COMMAND args)
	if(IOS AND RELEASE)
		set(args ${args} ${ARGN})
		IOS_COMMAND(${args})
	endif()
...
function(IOSSIM_DEBUG_COMMAND args)
	if(IOSSIM AND DEBUG)
		set(args ${args} ${ARGN})
		IOSSIM_COMMAND(${args})
	endif()
...
function(IOSSIM_RELEASE_COMMAND args)
	if(IOSSIM AND RELEASE)
		set(args ${args} ${ARGN})
		IOSSIM_COMMAND(${args})
	endif()
...
function(IOSSIM32_COMMAND args)
	if(IOSSIM_32)
		set(args ${args} ${ARGN})
		IOSSIM_COMMAND(${args})
	endif()
...
function(IOSSIM64_COMMAND args)
	if(IOSSIM_64)
		set(args ${args} ${ARGN})
		IOSSIM_COMMAND(${args})
	endif()
...
function(LINUX_DEBUG_COMMAND args)
	if(LINUX AND DEBUG)
		set(args ${args} ${ARGN})
		LINUX_COMMAND(${args})
	endif()
...
function(LINUX_RELEASE_COMMAND args)
	if(LINUX AND RELEASE)
		set(args ${args} ${ARGN})
		LINUX_COMMAND(${args})
	endif()
...
function(LINUX32_DEBUG_COMMAND args)
	if(LINUX_32 AND DEBUG)
		set(args ${args} ${ARGN})
		LINUX_COMMAND(${args})
	endif()
...
function(LINUX32_RELEASE_COMMAND args)
	if(LINUX_32 AND RELEASE)
		set(args ${args} ${ARGN})
		LINUX_COMMAND(${args})
	endif()
...
function(LINUX64_DEBUG_COMMAND args)
	if(LINUX_64 AND DEBUG)
		set(args ${args} ${ARGN})
		LINUX_COMMAND(${args})
	endif()
...
function(LINUX64_RELEASE_COMMAND args)
	if(LINUX_64 AND RELEASE)
		set(args ${args} ${ARGN})
		LINUX_COMMAND(${args})
	endif()
...
###############################
function(RASPBERRY_COMMAND args)
...
function(RASPBERRY_DEBUG_COMMAND args)
	if(RASPBERRY AND DEBUG)
		set(args ${args} ${ARGN})
...
function(RASPBERRY_RELEASE_COMMAND args)
	if(RASPBERRY AND RELEASE)
		set(args ${args} ${ARGN})
		RASPBERRY_COMMAND(${args})
	endif()
...
function(RASPBERRY32_COMMAND args)
	if(RASPBERRY_32)
		set(args ${args} ${ARGN})
		RASPBERRY_COMMAND(${args})
	endif()
...
function(RASPBERRY32_DEBUG_COMMAND args)
	if(RASPBERRY_32 AND DEBUG)
		set(args ${args} ${ARGN})
		RASPBERRY_COMMAND(${args})
	endif()
...
function(RASPBERRY32_RELEASE_COMMAND args)
	if(RASPBERRY_32 AND RELEASE)
		set(args ${args} ${ARGN})
		RASPBERRY_COMMAND(${args})
	endif()
...
function(RASPBERRY64_COMMAND args)
	if(RASPBERRY_64)
		set(args ${args} ${ARGN})
		RASPBERRY_COMMAND(${args})
	endif()
...
function(RASPBERRY64_DEBUG_COMMAND args)
	if(RASPBERRY_64 AND DEBUG)
		set(args ${args} ${ARGN})
		RASPBERRY_COMMAND(${args})
	endif()
...
function(RASPBERRY64_RELEASE_COMMAND args)
	if(RASPBERRY_64 AND RELEASE)
		set(args ${args} ${ARGN})
		RASPBERRY_COMMAND(${args})
	endif()
...
function(ANDROID_DEBUG_COMMAND args)
	if(ANDROID AND DEBUG)
		set(args ${args} ${ARGN})
...
function(ANDROID_RELEASE_COMMAND args)
	if(ANDROID AND RELEASE)
		set(args ${args} ${ARGN})
		ANDROID_COMMAND(${args})
	endif()
...
function(ANDROID32_COMMAND args)
	if(ANDROID_32)
		set(args ${args} ${ARGN})
		ANDROID_COMMAND(${args})
	endif()
...
function(ANDROID32_DEBUG_COMMAND args)
	if(ANDROID_32 AND DEBUG)
		set(args ${args} ${ARGN})
		ANDROID_COMMAND(${args})
	endif()
...
function(ANDROID32_RELEASE_COMMAND args)
	if(ANDROID_32 AND RELEASE)
		set(args ${args} ${ARGN})
		ANDROID_COMMAND(${args})
	endif()
...
function(ANDROID64_COMMAND args)
	if(ANDROID_64)
		set(args ${args} ${ARGN})
		ANDROID_COMMAND(${args})
	endif()
...
function(ANDROID64_DEBUG_COMMAND args)
	if(ANDROID_64 AND DEBUG)
		set(args ${args} ${ARGN})
		ANDROID_COMMAND(${args})
	endif()
...
function(ANDROID64_RELEASE_COMMAND args)
	if(ANDROID_64 AND RELEASE)
		set(args ${args} ${ARGN})
		ANDROID_COMMAND(${args})
	endif()
...
function(DKPLUGIN arg)
	...
	if(${arg} STREQUAL DK OR STATIC)
		file(GLOB HEADER_FILES RELATIVE ${DK}/DKPlugins ${plugin_path}/*.h)
