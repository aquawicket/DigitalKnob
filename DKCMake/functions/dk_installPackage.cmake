...
#
	...
	execute_process(COMMAND command -v brew OUTPUT_VARIABLE BREW_EXE)
	if(BREW_EXE)
		execute_process(COMMAND ${BREW_EXE} install ${package})			# Homebrew (MacOS)
	endif()

	execute_process(COMMAND command -v dnf OUTPUT_VARIABLE DNF_EXE)
	if(DNF_EXE)
		execute_process(COMMAND ${DNF_EXE} install ${package})			# Dnf (yum)
	endif()
	
	execute_process(COMMAND command -v emerge OUTPUT_VARIABLE EMERGE_EXE)
	if(EMERGE_EXE)
		execute_process(COMMAND ${EMERGE_EXE} ${package})				# Portage
	endif()
	
	execute_process(COMMAND command -v nix-env OUTPUT_VARIABLE NIX_ENV_EXE)
	if(NIX_ENV_EXE)
		execute_process(COMMAND ${NIX_ENV_EXE} -i ${package})					# Nix
	endif()
	
	execute_process(COMMAND command -v ohpm OUTPUT_VARIABLE OHPM_EXE)
	if(OHPM_EXE)
		execute_process(COMMAND ohpm install ${package})				# Ohpm
	endif()
	
	execute_process(COMMAND command -v pkg OUTPUT_VARIABLE PKG_EXE)
	if(PKG_EXE)
	...
	execute_process(COMMAND command -v swupd OUTPUT_VARIABLE SWUPD_EXE)
	if(SWUPD_EXE)
		execute_process(COMMAND ${SWUPD_EXE} bundle-add ${package})			# Swupd
	endif()
	
	execute_process(COMMAND command -v tce-load OUTPUT_VARIABLE TCE_LOAD_EXE)
	if(TCE_LOAD_EXE)
		execute_process(COMMAND ${TCE_LOAD_EXE} -wil ${package})     			# Tiny core linux
	endif()
	
	execute_process(COMMAND command -v winget OUTPUT_VARIABLE WINGET_EXE)
	if(WINGET_EXE)
		execute_process(COMMAND winget install ${package})				# WinGet
	endif()
	
	execute_process(COMMAND command -v xbps-install OUTPUT_VARIABLE WINGET_EXE)
	if(XBPS_INSTALL_EXE)
		execute_process(COMMAND xbps-install ${package})				# Xbps
	endif()
	
	execute_process(COMMAND command -v zypper OUTPUT_VARIABLE ZYPPER_EXE)
	if(ZYPPER_EXE)
