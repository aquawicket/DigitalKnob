...
function(dk_deleteTempFiles)
	...
	file(GLOB_RECURSE tmp_Files "${DKCPP_APPS_DIR}" "*.tmp")
	foreach(file ${tmp_Files})
		dk_info("dk_deleteTempFiles(): deleting ${file}. . .")
		#dk_delete("${file}")
	endforeach()
	...
	#if(WIN_HOST)
	#	dk_executeProcess(for /r %%Z in (*.tmp) do @if %%~xZ==.tmp del "%%Z" WORKING_DIRECTORY $ENV{DIGITALKNOB_DIR})
	#	dk_executeProcess(for /r %%Z in (*.TMP) do @if %%~xZ==.TMP del "%%Z" WORKING_DIRECTORY $ENV{DIGITALKNOB_DIR})
	#else()
	#	dk_executeProcess("rm -rf `find . -name *.tmp`" WORKING_DIRECTORY $ENV{DIGITALKNOB_DIR})
	#	dk_executeProcess(find . -name "*.TMP" -delete WORKING_DIRECTORY $ENV{DIGITALKNOB_DIR})
	#	dk_executeProcess(find . -name "*.tmp" -delete WORKING_DIRECTORY $ENV{DIGITALKNOB_DIR})	
	#endif()
endfunction()
