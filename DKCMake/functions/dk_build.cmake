	...
	#list(APPEND BUILD_MODE single_config)
		...
		if(EXISTS ${path}/${CONFIG_PATH}/cmake_install.cmake)
			...
			if(${ARGC} GREATER 1)
				...
				dk_if(DEBUG  "dk_command(${CMAKE_EXE} --build ${path}/${CONFIG_PATH} --config Debug --target ${target} --verbose)")# ${NO_HALT} OUTPUT_VARIABLE echo_output ECHO_OUTPUT_VARIABLE)
				#dk_fileAppend(${PWD}/DKBUILD.log "${echo_output}\n\n\n")
				
				dk_if(RELEASE "dk_command(${CMAKE_EXE} --build ${path}/${CONFIG_PATH} --config Release --target ${target} --verbose)")# ${NO_HALT})
			else()
				#dk_fileAppend(${PWD}/DKBUILD.log "${CMAKE_EXE} --build . --config Debug --target ${target} \n\n")
				dk_if(DEBUG "dk_command(${CMAKE_EXE} --build ${path}/${CONFIG_PATH} --config Debug --verbose)") # ${NO_HALT} OUTPUT_VARIABLE echo_output ECHO_OUTPUT_VARIABLE)")
				#dk_fileAppend(${PWD}/DKBUILD.log "${echo_output}\n\n\n")
				
				dk_if(RELEASE "dk_command(${CMAKE_EXE} --build ${path}/${CONFIG_PATH} --config Release --verbose)") # ${NO_HALT})")
			endif()
			dk_return()
		endif()
		if(MULTI_CONFIG)
			if(DEBUG)
				if(EXISTS ${path}/${triple}/${DEBUG_DIR}/cmake_install.cmake)
					...
					dk_info("Building with CMake")
					if(${ARGC} GREATER 1)
						dk_if(DEBUG "dk_command(${CMAKE_EXE} --build ${path}/${triple}/${DEBUG_DIR} --config Debug --target ${target} --verbose)") # ${NO_HALT})
					else()
						dk_if(DEBUG "dk_command(${CMAKE_EXE} --build ${path}/${triple}/${DEBUG_DIR} --config Debug --verbose)") # ${NO_HALT})
					endif()
					#dk_cd(${path}/${CONFIG_PATH})
					dk_return()
				endif()
			elseif(RELEASE)
				if(EXISTS ${path}/${triple}/${RELEASE_DIR}/cmake_install.cmake)
					...
					dk_info("Building with CMake")
					if(${ARGC} GREATER 1)
						dk_if(RELEASE "dk_command(${CMAKE_EXE} --build ${path}/${triple}/${RELEASE_DIR} --config Release --target ${target} --verbose)") # ${NO_HALT})
					else()
						dk_if(RELEASE "dk_command(${CMAKE_EXE} --build ${path}/${triple}/${RELEASE_DIR} --config Release --verbose)") # ${NO_HALT})
					endif()
					#dk_cd(${path}/${CONFIG_PATH})
					dk_return()
				endif()
