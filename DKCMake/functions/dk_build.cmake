...
function(dk_build path) #target NO_HALT
	...
	#list(APPEND BUILD_MODE single_config)
		...
		if(EXISTS ${path}/${Target_Config}/cmake_install.cmake)
			...
			if(target)
				dk_if(Debug  "dk_exec(${CMAKE_EXE} --build ${path}/${Target_Config} --config Debug --target ${target} --verbose)")
				dk_if(Release "dk_exec(${CMAKE_EXE} --build ${path}/${Target_Config} --config Release --target ${target} --verbose)")
			else()
				dk_if(Debug "dk_exec(${CMAKE_EXE} --build ${path}/${Target_Config} --config Debug --verbose)")
				dk_if(Release "dk_exec(${CMAKE_EXE} --build ${path}/${Target_Config} --config Release --verbose)")
			endif()
			dk_return()
		endif()
		if(MULTI_CONFIG)
