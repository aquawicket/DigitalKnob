...
'-  Clear
 ...
 '-- Function:     
'-  NumericReturn = ShowMsg  - returns number of  button was clicked.
 
'-- MsgBox demo:

Dim Box, R, s1, Pt1, sPar, sPath
...
With Box
  ...
  .Caption = "This is a sample message Box. Notice that it appears with default system colors."
  .AddButton "First"
  .AddButton "Second"
  .AddButton "Third"
  .AddList "", ""  '-- comment this line for multi-line listbox.
  .AddList "one", "hid one"
  .AddList "two", "hid two"
  .AddList "three", ""
  .AddList "four times something", ""
  .AddList "five", "hid five"
 ' .ListSize = 5  '-- comment this line to make listbox a single-line dropdown selector.
  .AddTextBox 30
  R = .ShowMsg
 End With
 
    MsgBox "Button pressed was: " & R
      If (R > 0) Then
          MsgBox "ListText: " & Box.ListText & vbCrLf & "ListData: " & Box.ListData & vbCrLf & "Index: " & Box.ListIndex & vbCrLf & "Text: " & Box.Text
      End If   
    
With Box
  .Clear  '-- reset for another use.
  .Title = "This one has an icon."
  If Len(sPar) > 0 Then .Icon = sPar & "icon1.gif"
  .IconWidth = 32
    s1 = "This is a second test of the custom MsgBox using an icon. If the icon didn't show you need to adjust the path. Note that If you use a very large image you should edit"
    s1 = s1 & " the self-sizing operation of the Box. It is not designed for formatting images over about 60 pixels."
  .Caption = s1
  .AddButton "This is Number one"
  .AddButton "OK"
...
With Box
  .Clear  '-- reset for another use.
  .Title = "Large icon"
 If Len(sPar) > 0 Then .Icon = sPar & "earth1.jpg"
  .IconWidth = 64
    s1 = "<H4>The Earth</H4> This MsgBox has a larger icon. It also shows how you can decorate the actual message text by using <B>inline HTML</B> formatting." 
   s1 = s1 & "This Box would be more balanced with more buttons to widen it. The self-sizing is not perfect for all uses."
.Caption = s1
  .AddButton "OK"
   .AddList "", ""
  .AddList "one", "hid one"
