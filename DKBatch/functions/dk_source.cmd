...
:dk_source
 ...
 setlocal
	...
	for %%Z in ("%DKBATCH_FUNCTIONS_DIR_%%~1.cmd") do set "_dirname_=%%~dpZ"
    ...
    :: Try powershell
    powershell /? %NO_STDOUT% && powershell -Command "(New-Object Net.WebClient).DownloadFile('%DKHTTP_DKBATCH_FUNCTIONS_DIR%/%~1.cmd', '%DKBATCH_FUNCTIONS_DIR_%%~1.cmd')"
    if exist "%DKBATCH_FUNCTIONS_DIR_%%~1.cmd" %return%
    
    :: Try dk_powershell
    %dk_call% dk_powershell -Command "(New-Object Net.WebClient).DownloadFile('%DKHTTP_DKBATCH_FUNCTIONS_DIR%/%~1.cmd', '%DKBATCH_FUNCTIONS_DIR_%%~1.cmd')"
    if exist "%DKBATCH_FUNCTIONS_DIR_%%~1.cmd" %return%
    
    :: Try curl
    curl --help %NO_STDOUT% && curl "%DKHTTP_DKBATCH_FUNCTIONS_DIR%/%~1.cmd" -o "%DKBATCH_FUNCTIONS_DIR_%%~1.cmd"
    if exist "%DKBATCH_FUNCTIONS_DIR_%%~1.cmd" %return%
    
    :: Try certutil
    certutil.exe /? %NO_STDOUT% && certutil.exe -urlcache -split -f "%DKHTTP_DKBATCH_FUNCTIONS_DIR%/%~1.cmd" "%DKBATCH_FUNCTIONS_DIR_%%~1.cmd"
    if exist "%DKBATCH_FUNCTIONS_DIR_%%~1.cmd" %return%
    
    if not exist "%DKBATCH_FUNCTIONS_DIR_%%~1.cmd" echo [31m failed to download %1.cmd [0m
%endfunction%
