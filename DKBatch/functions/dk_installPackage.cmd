...
%setlocal%
	...
	if defined PACMAN_EXE (
		...
		if defined Windows_X86_Clang (
			...
			&rem set(comand "${PACMAN_EXE}" -S mingw-w64-clang-i686-${package} --needed --noconfirm)								&rem CLANG32
			...
			&rem set(comand "${PACMAN_EXE}" -S mingw-w64-clang-x86_64-${package} --needed --noconfirm)								&rem CLANG64
			...
			&rem set(comand "${PACMAN_EXE}" -S mingw-w64-clang-aarch64-${package} --needed --noconfirm)								&rem CLANGARM64
			...
			&rem set(comand "${PACMAN_EXE}" -S mingw-w64-i686-${package} --needed --noconfirm)										&rem MINGW32
			...
			&rem set(comand "${PACMAN_EXE}" -S mingw-w64-x86_64-${package} --needed --noconfirm)									&rem MINGW64
			...
			&rem set(comand "${PACMAN_EXE}" -S mingw-w64-ucrt-x86_64-${package} --needed --noconfirm)								&rem UCRT64
		) else (
			set comand="${PACMAN_EXE}" -S ${package} --needed --noconfirm --cachedir "${MSYS2_CacheDir}")							&rem MSYS (DEFAULT)
			&rem set(comand "${PACMAN_EXE}" -S ${package} --needed --noconfirm)														&rem MSYS (DEFAULT)
		)
