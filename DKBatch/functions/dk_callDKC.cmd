...
setlocal enableDelayedExpansion
	...
	if "%target_env%"=="cosmocc" (
		...
		%dk_call% dk_validate COSMOCC_C_COMPILER	"%dk_call% %DKIMPORTS_DIR%/cosmocc/dk_install.cmd"
		%dk_call% dk_assertPath COSMOCC_C_COMPILER
		set "COMPILER_EXE=!SH_EXE! !COSMOCC_C_COMPILER!"
	)

	if "%target_env%"=="clang" (
		%dk_call% dk_validate CLANG_C_COMPILER		"%dk_call% %DKIMPORTS_DIR%/clang/dk_install.cmd"
		%dk_call% dk_assertPath CLANG_C_COMPILER
		set "COMPILER_EXE=!CLANG_C_COMPILER!"
	)
	if "%target_env%"=="gcc" (
		%dk_call% dk_validate GCC_C_COMPILER		"%dk_call% %DKIMPORTS_DIR%/gcc/dk_install.cmd"
	...
	if not exist "%DKC_BUILD_DIR%" (%dk_call% dk_makeDirectory "%DKC_BUILD_DIR%")
