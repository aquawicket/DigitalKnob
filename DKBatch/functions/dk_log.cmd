...
if not defined ERROR_TIMEOUT   set "ERROR_TIMEOUT=1"
...
setlocal
    ...
    if "%ENABLE_dk_log%" neq "1"  %return% 
        ...
        if "!%_level_%_LINE!"=="1"  	%dk_call% dk_echo "!%_level_%_COLOR!*** LINE_ON_%_level_% ***%clr%"   	&& %dk_call% dk_showFileLine "%_callerpath%" "%_message_%"
		if "!%_level_%_PAUSE!"=="1" 	%dk_call% dk_echo "!%_level_%_COLOR!*** PAUSE_ON_%_level_% ***%clr%"  	&& %dk_call% dk_pause
		if "!%_level_%_TIMEOUT!"=="1" 	%dk_call% dk_echo "!%_level_%_COLOR!*** TIMEOUT_ON_%_level_% ***%clr%"  && %dk_call% dk_timeout
