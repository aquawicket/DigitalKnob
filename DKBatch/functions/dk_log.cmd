...
::if not defined VERBOSE_TIMEOUT 	set "VERBOSE_TIMEOUT=1"
...
::if not defined DEBUG_TIMEOUT   	set "DEBUG_TIMEOUT=1"
...
::if not defined DEFAULT_TIMEOUT	set "DEFAULT_TIMEOUT=1"
...
::if not defined INFO_TIMEOUT  	  	set "INFO_TIMEOUT=1"
...
::if not defined SUCCESS_TIMEOUT  	set "SUCCESS_TIMEOUT=1"
...
::if not defined NOTICE_TIMEOUT  	set "NOTICE_TIMEOUT=1"
...
::if not defined WARNING_SOUND     	set "WARNING_SOUND=1"
...
::if not defined ERROR_SOUND     	set "ERROR_SOUND=1"
::if not defined ERROR_PAUSE   		set "ERROR_PAUSE=1"
if not defined ERROR_TIMEOUT   		set "ERROR_TIMEOUT=30"
...
::if not defined FATAL_TIMEOUT 		set "FATAL_TIMEOUT=1"
...
setlocal enableDelayedExpansion
    ...
    if "%ENABLE_dk_log%" neq "1"  %return% 
		...
		::### TRACE ###
		if "!%_level_%_TRACE!" gtr "0" %dk_call% dk_echo "!%_level_%_COLOR!*** TRACE_ON_%_level_% ***%clr%"  && %dk_call% dk_stacktrace
		
		::### LINE ###
		if "!%_level_%_LINE!" gtr "0"  %dk_call% dk_echo "!%_level_%_COLOR!*** LINE_ON_%_level_% ***%clr%"   && %dk_call% dk_showFileLine "%_callerpath%" "%_message_%"
		
		::### SOUND ###
		if "!%_level_%_SOUND!" gtr "0" %dk_call% dk_echo "!%_level_%_COLOR!*** SOUND_ON_%_level_% ***%clr%"  && (
		...
		::### PAUSE ###
		if "!%_level_%_PAUSE!" gtr "0" 		%dk_call% dk_echo "!%_level_%_COLOR!*** PAUSE_ON_%_level_% ***%clr%"  	&& %dk_call% dk_pause
		
		::### TIMEOUT ###
        if "!%_level_%_TIMEOUT!" gtr "0" 	%dk_call% dk_echo "!%_level_%_COLOR!*** TIMEOUT_ON_%_level_% ***%clr%"  && %dk_call% dk_timeout !%_level_%_TIMEOUT!
		
		::### HALT ###
		if "!%_level_%_HALT!" gtr "0"  		%dk_call% dk_echo "!%_level_%_COLOR!*** HALT_ON_%_level_% ***%clr%"   	&& %dk_call% dk_exit 13
%endfunction%
