...
:dk_generate () {
    ...
    set "CMAKE_ARGS="
    ::if "%TYPE%"=="Debug"           call dk_append CMAKE_ARGS -DDEBUG=ON & call dk_append CMAKE_ARGS -DRELEASE=OFF
    if "%TYPE%"=="Debug"             call dk_append CMAKE_ARGS -DDEBUG=ON
    ::if "%TYPE%"=="Release"         call dk_append CMAKE_ARGS -DDEBUG=OFF & call dk_append CMAKE_ARGS -DRELEASE=ON
    if "%TYPE%"=="Release"           call dk_append CMAKE_ARGS -DRELEASE=ON
    if "%TYPE%"=="All"               call dk_append CMAKE_ARGS -DDEBUG=ON & call dk_append CMAKE_ARGS -DRELEASE=ON
    if "%DKLEVEL%"=="Build"          call dk_append CMAKE_ARGS -DBUILD=ON
    if "%DKLEVEL%"=="Rebuild"        call dk_append CMAKE_ARGS -DREBUILD=ON
    if "%DKLEVEL%"=="RebuildAll"     call dk_append CMAKE_ARGS -DREBUILDALL=ON
    if "%DKLINK%"=="Static"          call dk_append CMAKE_ARGS -DSTATIC=ON
    if "%DKLINK%"=="Shared"          call dk_append CMAKE_ARGS -DSHARED=OFF
    ::if "%TARGET_OS%==emscripten" call dk_append CMAKE_ARGS -DEMSCRIPTEN=ON
        
    call dk_set CMAKE_BINARY_DIR "%CMAKE_TARGET_PATH%/%TARGET_OS%/%TYPE%"
    call dk_printVar CMAKE_BINARY_DIR
        
    call dk_append CMAKE_ARGS -S=%CMAKE_SOURCE_DIR%
    call dk_append CMAKE_ARGS -B=%CMAKE_BINARY_DIR%

    ::############ CMake Options ############
    ::call dk_append CMAKE_ARGS -DCMAKE_VERBOSE_MAKEFILE=1
    ::call dk_append CMAKE_ARGS -DCMAKE_COLOR_DIAGNOSTICS=ON
    call dk_append CMAKE_ARGS -Wdev
    ::call dk_append CMAKE_ARGS -Werror=dev
    call dk_append CMAKE_ARGS -Wdeprecated
    ::call dk_append CMAKE_ARGS -Werror=deprecated
    ::call dk_append CMAKE_ARGS --graphviz=graphviz.txt
    ::call dk_append CMAKE_ARGS --system-information system_information.txt
    ::call dk_append CMAKE_ARGS --debug-trycompile
    ::call dk_append CMAKE_ARGS --debug-output
    ::call dk_append CMAKE_ARGS --trace
    ::call dk_append CMAKE_ARGS --trace-expand
    ::call dk_append CMAKE_ARGS --warn-uninitialized
	...
	if "%TARGET_OS%"=="ios_arm32"          call dk_prepend CMAKE_ARGS -G Xcode
	if "%TARGET_OS%"=="ios_arm64"          call dk_prepend CMAKE_ARGS -G Xcode
	if "%TARGET_OS%"=="iossim_x86"         call dk_prepend CMAKE_ARGS -G Xcode
	if "%TARGET_OS%"=="iossim_x86_64"      call dk_prepend CMAKE_ARGS -G Xcode
	if "%TARGET_OS%"=="linux_x86"          call dk_prepend CMAKE_ARGS -G Unix Makefiles
	if "%TARGET_OS%"=="linux_x86_64"       call dk_prepend CMAKE_ARGS -G Unix Makefiles
	if "%TARGET_OS%"=="mac_x86"            call dk_prepend CMAKE_ARGS -G Xcode
	if "%TARGET_OS%"=="mac_x86_64"         call dk_prepend CMAKE_ARGS -G Xcode
	if "%TARGET_OS%"=="raspberry_arm32"    call dk_prepend CMAKE_ARGS -G Unix Makefiles
	if "%TARGET_OS%"=="raspberry_arm64"    call dk_prepend CMAKE_ARGS -G Unix Makefiles
...
::	if exist %TOOLCHAIN% call dk_append CMAKE_ARGS -DCMAKE_TOOLCHAIN_FILE=%TOOLCHAIN_FILE%
