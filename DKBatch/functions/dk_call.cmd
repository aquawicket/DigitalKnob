...
:dk_call
	...
	call :pushStack %*
::	if not defined __FILE__%LVL% (call :setGlobal __FILE__%LVL% %DKSCRIPT_PATH%)
::	(set /a LVL+=1)
::	if not defined __CMND__%LVL% (call :setGlobal __CMND__%LVL% %DKSCRIPT_PATH%)
::	if not defined __FILE__%LVL% (call :setGlobal __FILE__%LVL% %DKSCRIPT_PATH%)
::	if not defined __FUNC__%LVL% (call :setGlobal __FUNC__%LVL% %DKSCRIPT_NAME%)
::	if not defined __ARGV__%LVL% (call :setGlobal __ARGV__%LVL% %DKSCRIPT_ARGS%)
::	(set /a LVL+=1)
::	if not defined __CMND__%LVL% (call :setGlobal __CMND__%LVL% %DKBATCH_FUNCTIONS_DIR_%DK.cmd)
::	if not defined __FILE__%LVL% (call :setGlobal __FILE__%LVL% %DKBATCH_FUNCTIONS_DIR_%DK.cmd)
::	if not defined __FUNC__%LVL% (call :setGlobal __FUNC__%LVL% DK)
::	if not defined __ARGV__%LVL% (call :setGlobal __ARGV__%LVL% %~0 %*)
