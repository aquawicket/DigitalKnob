...
::%dk_call% dk_debugFunc 1 9
	...
	if not defined PLUGIN.IMPORT_NAME_Upper (
	...
	if not defined PLUGIN.IMPORT_NAME_Alphanumeric (
	...
	::### <PLUGIN.ID>.BRANCH
	::set "%PLUGIN.ID%.BRANCH=%PLUGIN.BRANCH%"
	::%dk_call% dk_echo "%PLUGIN.ID%.BRANCH = '!%PLUGIN.ID%.BRANCH!'"

	::### <PLUGIN.ID>.TAG
	::set "%PLUGIN.ID%.TAG=%PLUGIN.TAG%"
