@echo off
call %DKBATCH_FUNCTIONS_DIR_%DK.cmd

::####################################################################
::# dk_callPowershell(function, arguments..., return_args...)
::#
::#
:dk_callPowershell
	call dk_debugFunc
	if %__ARGC__% lss 1 call dk_error "%__FUNCTION__%:%__ARGV__% incorrect number of arguments"
	
	call dk_validate DKPOWERSHELL_FUNCTIONS_DIR "call dk_validateBranch"
	
	:: https://stackoverflow.com/a/4732316/688352
	cmd /c powershell Set-ExecutionPolicy RemoteSigned -Scope CurrentUser
	
	echo for /f "usebackq delims=|" %%Z in (`powershell %DKPOWERSHELL_FUNCTIONS_DIR%\%~1.ps1`) do echo %%Z
	for /f "usebackq delims=|" %%Z in (`powershell %DKPOWERSHELL_FUNCTIONS_DIR%\%~1.ps1`) do echo %%Z

	call dk_echo "PSValue received from Powershell : %PSValue%"
goto:eof




::###### DKTEST ###### DKTEST ###### DKTEST ###### DKTEST ###### DKTEST ######
:DKTEST
	call dk_debugFunc
	
	call dk_callPowershell dk_debug "C:\Windows\System32"
goto:eof
