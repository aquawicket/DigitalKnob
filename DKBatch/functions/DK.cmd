...
:dk_initFiles
	...
	if EXIST "%DKARCHIVE%" (
		...
		if NOT EXIST "%DKBATCH_FUNCTIONS_DIR_%dk_source.cmd" 			(tar -zxf %DKARCHIVE% -C %DKBRANCH_DIR% DKBatch/functions/dk_source.cmd)
		if NOT EXIST "%DKBATCH_FUNCTIONS_DIR_%dk_call.cmd" 				(tar -zxf %DKARCHIVE% -C %DKBRANCH_DIR% DKBatch/functions/dk_call.cmd)
		if NOT EXIST "%DKBATCH_FUNCTIONS_DIR_%dk_return.cmd" 			(tar -zxf %DKARCHIVE% -C %DKBRANCH_DIR% DKBatch/functions/dk_return.cmd)
		if NOT EXIST "%DKBATCH_FUNCTIONS_DIR_%dk_printLastError.cmd" 	(tar -zxf %DKARCHIVE% -C %DKBRANCH_DIR% DKBatch/functions/dk_printLastError.cmd)
	) else (
